	        ...
	        if(Wscode == 200 && meta != null && Wsstatus.equalsIgnoreCase("SUCCESS") && timestamp != null)
	        {
	    		...
	    		if(js.getString("data.langCd")!=null)
	    		{
	    			...
	    			if(internalMsg.equals(expectMessage))
	    			{
	            		...
	            		String[] inputFieldNames = {"Input_UserName: ","Input_langCd: ", "Input_englLanguageNm: ","Input_nativeScriptLanguageNm:","Input_LastUpdateUserName: "
	            		...
	            		String[] dbFieldNames = {"DB_UserName: ","DB_langCd: ", "DB_englLanguageNm: ","DB_nativeScriptLanguageNm:","DB_LastUpdateUserName: "
	...
	{	
		...
		try {
	        ...
	        if(Wscode == 400 && meta != null && timestamp != null)
		    {
				...
				//String expectMessage2 = resMsgs.dowNbrForPutBlankMsg2;
	...
	{	
		...
		try {
	        ...
	        if(Wscode == 400 &&  meta != null && timestamp != null)
		    {
				...
				//String expectMessage2 = resMsgs.moyNbrForPutBlankMsg2;
		...
		try {
	        ...
	        if(Wscode == 400 &&  meta != null && timestamp != null)
		    {
				...
				if(errorMsg2.get(0).equals(expectMessage))
				{
	        		...
	        		test.info("Input Data Values:");
            		test.info(writableInputFields.replaceAll("\n", "<br />"));
	        		logger.info("Expected error message is getting received in response when same JSON request is processed again");
	        		logger.info("Execution is completed for Passed Test Case No. "+testCaseID);
	    			logger.info("------------------------------------------------------------------");
	        		test.pass("Expected error message is getting received in response when same JSON request is processed again");
	        		ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted,	writableInputFields, "NA",
	    					Wsstatus, ""+Wscode, responsestr1, "Pass", "" );
					test.log(Status.PASS, MarkupHelper.createLabel("test status", ExtentColor.GREEN));
				}else {
		...
		try {
	        ...
	        if(Wscode == 400 && meta != null && timestamp != null)
		    {
				...
				if(errorMsg1.get(0).equals("langCd") && errorMsg2.get(0).equals(expectMessage))
				{
	        		...
	        		test.info("Input Data Values:");
            		test.info(writableInputFields.replaceAll("\n", "<br />"));
	        		logger.info("Expected error message is getting received in response when languageCd is not passed in JSON request");
	        		logger.info("Execution is completed for Passed Test Case No. "+testCaseID);
	    			logger.info("------------------------------------------------------------------");
	        		test.pass("Expected error message is getting received in response when languageCd is not passed in JSON request");
	        		ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted,	writableInputFields, "NA",
	    					Wsstatus, ""+Wscode, responsestr1, "Pass", "" );
					test.log(Status.PASS, MarkupHelper.createLabel("test status", ExtentColor.GREEN));
				}else {

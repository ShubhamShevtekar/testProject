...
public class MonthOfYearGraphQL extends Reporting{
	...
	{	
		...
		try {
        	...
        	//** without parameter
          String payload="{\"query\":\"{  monthsOfYear {    monthOfYearNumber    monthOfYearShortName  }}\"}";
	...
	{	
		...
		try {
        	...
        	//** without parameter
            String payload="{\"query\":\"{  monthsOfYear {    monthOfYearNumbe    monthOfYearShortName  }}\"}";
        	//***send request and get response
			Response res = GetResponse.sendGraphQLRequest(payload,tokenValues[0], token, getEndPoinUrl, fileName, testCaseID);
			String responsestr=res.asString(); 
			String responsestr1 = Miscellaneous.jsonFormat(responsestr);
	        		...
	        		&& actualRespVersionNum.equalsIgnoreCase(actuatorGraphQLversion))
		    {
				...
				if(errorMsg1.get(0).equals("ValidationError") && errorMsg2.get(0).equals(expectMessage))
				{
	        		...
	        		logger.info("Expected error message is getting received in response when sending the invalid MonthOfYearNumber atrribute in request body");
	        		logger.info("Execution is completed for Passed Test Case No. "+testCaseID);
	    			logger.info("------------------------------------------------------------------");
	        		test.pass("Expected error message is getting received in response when sending the invalid MonthOfYearNumber atrribute in request body");
		        	...
		        	ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, payload,"", "", Wsstatus, ""+Wscode,
	...
	{	
		...
		try {
        	...
        	//** without parameter
           String payload="{\"query\":\"{  monthsOfYear {    monthOfYearNumbe    monthOfYearShortNam  }}\"}";
        	//***send request and get response
			Response res = GetResponse.sendGraphQLRequest(payload,tokenValues[0], token, getEndPoinUrl, fileName, testCaseID);
			String responsestr=res.asString(); 
			String responsestr1 = Miscellaneous.jsonFormat(responsestr);
	        		...
	        		&& actualRespVersionNum.equalsIgnoreCase(actuatorGraphQLversion))
		    {
					...
					&&	(errorMsg1.get(1).equals("ValidationError") && errorMsg2.get(1).equals(expectMessage1)))
				{
					...
					String[] inputFieldNames = {"Input_monthOfYearNumber: ", "Input_monthOfYearShortName: "};
	        		writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
	        		logger.info("Expected error message is getting received in response  passing the multiple (2 attribute) invalid attribute name in request body.");
	        		logger.info("Execution is completed for Passed Test Case No. "+testCaseID);
	    			logger.info("------------------------------------------------------------------");
	        		test.pass("Expected error message is getting received in response  passing the multiple (2 attribute) invalid attribute name in request body.");
	        		ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, payload,	"NA", "NA",
	    					Wsstatus, ""+Wscode, responsestr1, "Pass", "" );
					test.log(Status.PASS, MarkupHelper.createLabel("test status", ExtentColor.GREEN));
				}else {
		        	...
		        	ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, payload,"NA", "NA", Wsstatus, ""+Wscode,
							responsestr, "Fail", "ValidationError"+expectMessage+"  ValidationError"+expectMessage1 );
		        	Assert.fail("Test Failed");
		        }
		    }else {
	        	if(Wscode!=200){

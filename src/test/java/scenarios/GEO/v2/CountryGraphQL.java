...
public class CountryGraphQL extends Reporting{
	...
	countryNumberCd,countryCode,threeCharacterCountryCode,independentFlag,postalFormatDescription,postalFlag,postalLengthNumber,
	...
	{	
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			//***send the data to create request and get request
			String payload = "{\"query\":\"{  countries(geopoliticalId : \\\""+geopoliticalId+"\\\") {    geopliticalId    countryNumercCode    countryCode    threeCharacterCountryCode    independantFlag    dependentRelationshipId    dependantCountryCode    internationalDialingCode      mobilePhoneMinimumLengthNumber      mobilePhoneMaximumLengthNumber      landPhoneMinimumLengthNumber      landPhoneMaximumLengthNumber      postalFormatDescription    postalFlag    postalLengthNumber    firstWorkWeekDayName    lastWorkWeekDayName    weekendFirstDayName    internetDomainName    effectiveDate    expirationDate    countryDialings {      geopoliticalId      internationalDialingPrefixCode      effectiveDate      expirationDate    }    currencies {      geopoliticalId      currencyNumericCode      currencyCode      minorUnitCode      moneyFormatDescription      effectiveDate      expirationDate    }    geopoliticalUnitOfMeasures {      uomTypeCode      geopoliticalId      effectiveDate      expirationDate      refUOMType {        uomTypeCode        uomTypeName        uomTypeDesc      }    }    geopoliticalHolidays {      geopoliticalId      holidayId      effectiveDate      expirationDate      holiday {        holidayId        holidayName        holidayDateParameterText      }    }    geopoliticalAffiliations {      affilTypeId      geopoliticalId      effectiveDate      expirationDate      geoPoliticalAffiliatedType {        affilTypeId        affilTypeCode        affilTypeName      }    }    locales {      localeCode      geopoliticalId      languageCode      scriptCode      dateFullFormatDescription      dateLongFormatDescription      dateMediumFormatDescription      dateShortFormatDescription      cldrVersionNumber      cldrVersionDate      effectiveDate      expirationDate      languages {        languageCode        threeCharacterLanguageCode        languageName        nativeScriptLanguageName        scriptCode        languageDescription      }    }    translatedGeopoliticals {      geopoliticalId      languageCode      scriptCode      translatedName      versionNumber      versionDate      effectiveDate      expirationDate    }    countryOrgStd {      geopoliticalId      countryShortName      countryFullName      organisationStandardCode      effectiveDate      expirationDate      geoPoliticalOrganizationStandard {        organisationStandardCode        organisationStandardName      }    }    geopoliticalAreas {      geopoliticalId      geopoliticalTypeId      effectiveDate      expirationDate      geoPoliticalType {        geopoliticalTypeId        geopoliticalTypeName      }    }    dependentCountryRelationship {      dependentRelationshipId      dependentRelationshipDescription    }    stateProvStndList {      geopoliticalId      organisationStandardCode      stateProvinceCode      stateProvinceName      effectiveDate      expirationDate    }  }}\"}";
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			test.info(reqFormatted.replaceAll("\n", "<br />"));
			//***get end point url
	        ...
	        int Wscode= res.statusCode();
	        String expectMessage1 = resMsgs.countryInvalidAttributeGraphQLMsg2;
	        ...
	        if(Wscode == 200  && meta != null && (!meta.contains("timestamp")) && actualRespVersionNum.equalsIgnoreCase(actuatorGraphQLversion))
	        {
	    		...
	    		if(getResultDB.size() == responseRows.size()*fields.size())
	    		{
		        	...
		        	for(int i=responseRows.size()-1; i>=0; i=i-1)
		        	{
		        		...
		        		if(StringUtils.isBlank(js.getString("data.countries["+i+"].postalLengthNumber")))
		        		{
		        			getResponseRows.add("");
		        		}else {
		        			getResponseRows.add(js.getString("data.countries["+i+"].postalLengthNumber"));
		        		}
		        		...
		        		if(StringUtils.isBlank(js.getString("data.countries["+i+"].landPhoneMaximumLengthNumber")))
		        		{
		        			getResponseRows.add("");
		        		}else {
		        			getResponseRows.add(js.getString("data.countries["+i+"].landPhoneMaximumLengthNumber"));
		        		}
		        		if(StringUtils.isBlank(js.getString("data.countries["+i+"].landPhoneMinimumLengthNumber")))
		        		{
		        			getResponseRows.add("");
		        		}else {
		        			getResponseRows.add(js.getString("data.countries["+i+"].landPhoneMinimumLengthNumber"));
		        		}
		        		if(StringUtils.isBlank(js.getString("data.countries["+i+"].mobilePhoneMaximumLengthNumber")))
		        		{
		        			getResponseRows.add("");
		        		}else {
		        			getResponseRows.add(js.getString("data.countries["+i+"].mobilePhoneMaximumLengthNumber"));
		        		}
		        		if(StringUtils.isBlank(js.getString("data.countries["+i+"].mobilePhoneMinimumLengthNumber")))
		        		{
		        			getResponseRows.add("");
		        		}else {
		        			getResponseRows.add(js.getString("data.countries["+i+"].mobilePhoneMinimumLengthNumber"));
		        	...
		        	for(int j=0; j<getResultDB.size(); j=j+fields.size())
		        	{
		        				...
		        				)
			        	{
		        			...
		        			if(testCaseID!= "TC_09")
		        			{
			        			...
			        			test.info("Dependent Country Relationship validation starts:");
			        			countryDepnCntryRltspValidation(responsestr, testCaseID, z-1, targetDate , endDate);
			        			test.info("StateProvStd validation starts:");
			        			countryStateProvStdValidation(responsestr, testCaseID, z-1, targetDate , endDate);
		        			}
		        			z++;
			        	}else {
			        		String[] responseDbFieldValues = {
...
//        	List<String> getResponseRows = new ArrayList<>();
	...
	private void countryOrgStdValidation(String responsestr, String testCaseID, int k, String targetDate , String endDate) {
		...
		List<String> getResultDBFinal = new ArrayList<String>();
		List<String> responseRows = null;
		if(js.get("data.countries["+k+"].countryOrgStd")!=null)
		{
			responseRows = js.get("data.countries["+k+"].countryOrgStd");
			geopoliticalId = js.get("data.countries["+k+"].geopoliticalId").toString();
		...
		if(getResultDB.size() == responseRows.size()*fields.size())
		{
        	...
        	for(int i=0; i<responseRows.size(); i++)
        	{
            	...
            	String resGeoplId = js.getString("data.countries["+k+"].countryOrgStd["+i+"].geopoliticalId");
            	String resOrgStdCd1 = js.getString("data.countries["+k+"].countryOrgStd["+i+"].organizationStandardCode");
            	String resCountryFullNm = js.getString("data.countries["+k+"].countryOrgStd["+i+"].countryFullName");
            	String resCountryShortNm = js.getString("data.countries["+k+"].countryOrgStd["+i+"].countryShortName");
            	String resOrgStdCd2 = js.getString("data.countries["+k+"].countryOrgStd["+i+"].geoPoliticalOrganizationStandard.organizationStandardCode");
            	String resOrgStdNm = js.getString("data.countries["+k+"].countryOrgStd["+i+"].geoPoliticalOrganizationStandard.organizationStandardName");
            	String resEffDate = js.getString("data.countries["+k+"].countryOrgStd["+i+"].effectiveDate");
            	String resExpDate = js.getString("data.countries["+k+"].countryOrgStd["+i+"].expirationDate");
	...
	private void countryOrgStdValidationWithDates(String responsestr, String testCaseID, int k, String targetDate, String endDate) {
		...
		if(getResultDB.size() == responseRows.size()*fields.size())
		{
        	...
        	for(int i=0; i<responseRows.size(); i++)
        	{
        		if(StringUtils.isBlank(js.getString("data.countries["+k+"].countryOrgStd["+i+"].orgStdNm")))
        		{
        			getResponseRows.add("");
        		}else {
        			getResponseRows.add(js.getString("data.countries["+k+"].countryOrgStd["+i+"].orgStdNm").trim());
        		}
        		if(StringUtils.isBlank(js.getString("data.countries["+k+"].countryOrgStd["+i+"].countryFullName")))
        		{
        			getResponseRows.add("");
        		}else {
        			getResponseRows.add(js.getString("data.countries["+k+"].countryOrgStd["+i+"].countryFullName").trim());
        		}
        		if(StringUtils.isBlank(js.getString("data.countries["+k+"].countryOrgStd["+i+"].countryShortName")))
        		{
        			getResponseRows.add("");
        		}else {
        			getResponseRows.add(js.getString("data.countries["+k+"].countryOrgStd["+i+"].countryShortName").trim());
        		}
        		if(StringUtils.isBlank(js.getString("data.countries["+k+"].countryOrgStd["+i+"].effectiveDate")))
        		{
        			getResponseRows.add("");
        		}else {
        			String str = js.getString("data.countries["+k+"].countryOrgStd["+i+"].effectiveDate");
        			//int index = str.indexOf("T");
        			str = str.substring(0, 10);
        			getResponseRows.add(str);
        		}
        		if(StringUtils.isBlank(js.getString("data.countries["+k+"].countryOrgStd["+i+"].expirationDate")))
        		{
        			getResponseRows.add("");
        		}else {
        			String str = js.getString("data.countries["+k+"].countryOrgStd["+i+"].expirationDate");
        			str = str.substring(0, 10);
        			getResponseRows.add(str);
        		}
        	}

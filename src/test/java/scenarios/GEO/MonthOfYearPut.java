...
/*		tokenValues = RetrieveEndPoints.getTokenProperties(fileName);
	        ...
	        if(Wscode == 200 && meta != null && Wsstatus.equalsIgnoreCase("SUCCESS") && timestamp != null)
	        {
	    		...
	    		if(js.getString("data.monthOfYearNumber")!=null)
	    		{
	    			...
	    			if(internalMsg.equals(expectMessage))
	    			{
	            		...
	            		String[] inputFieldNames = {"Input_monthOfYearNumber: ", "Input_monthOfYearShortName: ", "Input_LastUpdateUserName: "};
	            		writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
	            		String[] dbFieldNames = {"DB_monthOfYearNumber: ", "DB_monthOfYearShortName: ", "DB_LastUpdateUserName: "};
	...
	//***error message validation
	        	...
	        	if(errorMsg1.get(0).equals("Error") && errorMsg2.get(0).equals(expectMessage))
				{
					...
					String[] inputFieldNames = {"Input_UserName: ", "Input_monthOfYearNumber: ", "Input_monthOfYearShortName: "};
	        		writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
	        		logger.info("Expected error message is getting received in response when the invalid monthofyearNumber (not exist in DB) is passed in JSON");
	        		logger.info("Execution is completed for Passed Test Case No. "+testCaseID);
	    			logger.info("------------------------------------------------------------------");
	        		test.pass("Expected error message is getting received in response when the invalid monthofyearNumber (not exist in DB) is passed in JSON");
	        		ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted,	writableInputFields, "NA",
	    					Wsstatus, ""+Wscode, responsestr1, "Pass", "" );
					test.log(Status.PASS, MarkupHelper.createLabel("test status", ExtentColor.GREEN));
				}else {
	...
	{	
		...
		try {
	       ...
	       // String internalMsg = js.getString("errorMessages.e");
			...
			List<String> errorMsg1 = new ArrayList<>();
			List<String> errorMsg2 = new ArrayList<>();
			String expectMessage = resMsgs.jsonParserErrorMsg;
			for(int i=0; i<errorMsgLength; i++){
				errorMsg1.add(js.getString("errors["+i+"].fieldName"));
				errorMsg2.add(js.getString("errors["+i+"].message"));
			}
			 ...
			 if(Wscode == 400 &&  meta != null && timestamp != null)
		    {
	        	...
	        	if(errorMsg1.get(0).equals("Error") && errorMsg2.get(0).equals(expectMessage))
				{
					...
					String[] inputFieldNames = {"Input_UserName: ", "Input_monthOfYearNumber: ", "Input_monthOfYearShortName: "};
	        		writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
	        		logger.info("Expected error message is getting received in response when the invalid monthofyearNumber (alphabets) is passed in JSONr");
	        		logger.info("Execution is completed for Passed Test Case No. "+testCaseID);
	    			logger.info("------------------------------------------------------------------");
	        		test.pass("Expected error message is getting received in response when the invalid monthofyearNumber (alphabets) is passed in JSONr");
	        		ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted,	writableInputFields, "NA",
	    					Wsstatus, ""+Wscode, responsestr1, "Pass", "" );
					test.log(Status.PASS, MarkupHelper.createLabel("test status", ExtentColor.GREEN));
				}else {
	...
	//***error message validation
	        	...
	        	if(errorMsg1.get(0).equals("Error") && errorMsg2.get(0).equals(expectMessage))
				{
					...
					String[] inputFieldNames = {"Input_UserName: ", "Input_monthOfYearNumber: ", "Input_monthOfYearShortName: "};
	        		writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
	        		logger.info("Expected error message is getting received in response when the invalid monthofyearNumber (special characters) is passed in JSON");
	        		logger.info("Execution is completed for Passed Test Case No. "+testCaseID);
	    			logger.info("------------------------------------------------------------------");
	        		test.pass("Expected error message is getting received in response when the invalid monthofyearNumber (special characters) is passed in JSON");
	        		ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted,	writableInputFields, "NA",
	    					Wsstatus, ""+Wscode, responsestr1, "Pass", "" );
					test.log(Status.PASS, MarkupHelper.createLabel("test status", ExtentColor.GREEN));
				}else {

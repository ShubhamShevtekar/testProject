...
public class GeoRltspTypeGraphQL extends Reporting {
	String scenarioName = getClass().getSimpleName();
	String TestCaseDescription, scenarioType, geoRsTypeCode, geoRsTypeDesc;
	...
	static Logger logger = Logger.getLogger(GeoRltspTypeGraphQL.class);
	...
	public void TC_01() {
		...
		try {
			...
			String payload = PostMethod.geoRsTypeGraphQlRequestWithoutParameters();
			...
			String internalMsg = js.getString("data.relationshipTypes");
			List<String> responseRows = js.get("data.relationshipTypes");
				...
				if (getResultDB.size() == responseRows.size() * fields.size()) {
					...
					for (int i = 0; i < responseRows.size(); i++) {
						if (StringUtils.isBlank(
								js.getString("data.relationshipTypes[" + i + "].geopoliticalRelationshipTypeCode"))) {
							getResponseRows.add("");
						} else {
							getResponseRows.add(
									js.getString("data.relationshipTypes[" + i + "].geopoliticalRelationshipTypeCode"));
						...
						if (StringUtils.isBlank(
								js.getString("data.relationshipTypes[" + i + "].areaRelationshipTypeDescription"))) {
							getResponseRows.add("");
						} else {
							getResponseRows.add(
									js.getString("data.relationshipTypes[" + i + "].areaRelationshipTypeDescription"));
					...
					for (int j = 0; j < getResultDB.size(); j = j + fields.size()) {
						if (getResultDB.get(j).toString().equals(getResponseRows.get(j).toString())
							...
							String[] responseDbFieldNames = { "Response_relationshipTypeCode: ",
									"DB_relationshipTypeCode: ", "Response_areaRelationshipTypeDesc: ",
									"DB_areaRelationshipTypeDesc: " };
	...
	public void TC_02() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.geoRsTypeGraphQlRequest(geoRsTypeCode);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			// ***get end point url
			String getEndPoinUrl = RetrieveEndPoints.getEndPointUrl("graphQLGet", fileName, level + ".GraphQL");
			logger.info("URI passed: " + getEndPoinUrl);
			...
			if (Wscode == 200 && meta != null && (!meta.contains("timestamp"))&& actualRespVersionNum.equalsIgnoreCase("1.0.0")) {
				...
				test.pass("Response timestamp validation passed");test.pass("Response API version number validation passed");
				// ***get the DB query
				String geoRsTypeGetQuery = query.geoRsTypePostQuery(geoRsTypeCode);
				// ***get the fields needs to be validate in DB
				List<String> fields = ValidationFields.geoRsTypeGetMethodDbFields();
				// ***get the result from DB
				List<String> getResultDB = DbConnect.getResultSetFor(geoRsTypeGetQuery, fields, fileName, testCaseID);
	...
	public void TC_03() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.geoRsTypeGraphQlRequestWithInvalidAttributeName(geoRsTypeCode);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			// logger.info("Input Request created:");
			test.info("Input Request created:");
			// logger.info(reqFormatted.replaceAll("\n", "<br />"));
			...
			String Wsstatus= res.getStatusLine();
	        int Wscode= res.statusCode();
	        String expectMessage1 = resMsgs.geoRsTypeCdFieldNotPresent;
	        String expectMessage2 = resMsgs.geoRsTypeDescFieldNotPresent;
	        String meta = js.getString("meta");String actualRespVersionNum = js.getString("meta.version");
	       	...
	       	if(Wscode == 200 && meta!=null && (!meta.contains("timestamp"))&& actualRespVersionNum.equalsIgnoreCase("1.0.0"))
		    {
				...
				{
					String[] inputFieldValues = {geoRsTypeCode, geoRsTypeDesc};
	        		String[] inputFieldNames = {"Input_GeoRsTypeCode: ", "Input_GeoRsTypeDesc: "};
		        	...
		        	ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted,"", "", Wsstatus, ""+Wscode,
							responsestr, "Fail", "geopoliticalRelationshipTypeCd"+expectMessage1 );
		        	Assert.fail("Test Failed");
		        }
		    } else {
	        	if(Wscode!=200){
	...
	public void TC_04() {
		...
		try {
			...
			String meta = js.getString("meta");String actualRespVersionNum = js.getString("meta.version");
			String Wsstatus = js.getString("meta.status");
			js.getString("data.relationshipTypes");
			List<String> responseRows = js.get("data.relationshipTypes");
			int Wscode = res.statusCode();
			...
			if (Wscode == 200 && meta != null && (!meta.contains("timestamp"))&& actualRespVersionNum.equalsIgnoreCase("1.0.0")) {
	        	...
	        	if(responseRows.size() == 0 )
				{
	        		...
	        		String[] inputFieldNames = {"Input_GeoRsTypeCode: ", "Input_GeoRsTypeDesc: "};
	        		writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
	        		logger.info("No record is getting fetched for the given invalid Relation Type Code");
	    			logger.info("Execution is completed for Passed Test Case No. "+testCaseID);
	    			logger.info("------------------------------------------------------------------");
	        		test.pass("No record is getting fetched for the given invalid Relation Type Code");
	        		ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted,	writableInputFields, "NA",
	    					Wsstatus, ""+Wscode, responsestr1, "Pass", "" );
					test.log(Status.PASS, MarkupHelper.createLabel("test status", ExtentColor.GREEN));
				}else {
		        	...
		        	test.fail("Expected error message is not getting received in response");
		        	ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted,"", "", Wsstatus, ""+Wscode,
							responsestr, "Fail", "geopoliticalRelationshipTypeCd" );
		        	Assert.fail("Test Failed");
		        }
		    }  else {
	        	if(Wscode!=200){

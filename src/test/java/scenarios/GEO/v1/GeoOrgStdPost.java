		...
		try {
			...
			if (Wscode == 200 && meta != null && Wsstatus.equalsIgnoreCase("SUCCESS")
				...
				if (js.getString("data.organizationStandardCode") != null) {
					...
					String expectMessage = resMsgs.geoOrgStdPostNewSuccessMsg + geoOrgStdCode1;
					if (internalMsg.equals(expectMessage) && geoOrgStdCode1.equals(geoOrgStdCode)) {
						logger.info(
								"Success message with Geopitical Org Std Cosde is getting received as expected in response");
						test.pass(
								"Success message with Geopitical Org Std Cosde is getting received as expected in response");
					} else {
					...
					String[] dbFieldNames = { "DB_UserName: ", "DB_GeoOrgStdCode: ", "DB_GeoOrgStdName: ",
					...
					&& actualRespVersionNum.equalsIgnoreCase("1.0.0")) {
				{
					...
					if (js.getString("data.organizationStandardCode") != null) {
						...
						testResult = TestResultValidation.testValidationWithDB(res, inputFieldValues, getResultDB,
								resFields);
						// ***write result to excel
						String[] inputFieldNames = { "Input_UserName: ", "Input_GeoOrgStdCode: ",
								"Input_GeoOrgStdName: ", "Input_LastUpdateUserName: " };
						writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
						String[] dbFieldNames = { "DB_UserName: ", "DB_GeoOrgStdCode: ", "DB_GeoOrgStdName: ",
								"DB_LastUpdateUserName: " };
						writableDBFields = Miscellaneous.geoDBFieldNames(getResultDB, dbFieldNames);

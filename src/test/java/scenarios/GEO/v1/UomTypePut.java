...
public class UomTypePut extends Reporting {
	...
	String TestCaseDescription, scenarioType, userId, uomTypeCode, uomTypeName, uomTypeDesc;// ,
																							// token;
		...
		try {
			...
			if (Wscode == 200 && meta != null && Wsstatus.equalsIgnoreCase("SUCCESS")
				...
				if (js.getString("data.uomTypeCode") != null) {
					...
					String expectMessage = resMsgs.uomTypePutSuccessNewMsg + uomTypeCode1;
					if (internalMsg.equals(expectMessage) && uomTypeCode1.equals(uomTypeCode)) {
						...
						String[] inputFieldNames = { "Input_uomTypeCode: ", "Input_uomTypeName: ",
								"Input_uomTypeDescription: ", "Input_LastUpdateUserName: " };
						writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
						String[] dbFieldNames = { "DB_uomTypeCode: ", "DB_uomTypeName: ", "DB_uomTypeDescription: ",
		...
		try {
			...
			if (Wscode == 400 && meta != null && actualRespVersionNum.equalsIgnoreCase(actuatorcommandversion)) {
				...
				if (errorMsg1.get(0).equals("uomTypeCode") && errorMsg2.get(0).equals(expectMessage)) {
					...
					String[] inputFieldNames = { "Input_UserName: ", "Input_uomTypeCode: ", "Input_uomTypeName: ",
					...
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when sending the blank UomType Code");
					ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted,
					...
					ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "",
							Wsstatus, "" + Wscode, responsestr, "Fail", "uomTypeCd" + expectMessage);
					Assert.fail("Test Failed");
				}
			} else {
				if (Wscode != 400) {
				...
				ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "", Wsstatus,
						"" + Wscode, responsestr, "Fail", "uomTypeCd" + expectMessage);
				Assert.fail("Test Failed");
			}
		} catch (Exception e) {
			e.printStackTrace();
		...
		try {
			...
			if (Wscode == 400 && meta != null && actualRespVersionNum.equalsIgnoreCase(actuatorcommandversion)) {
				...
				if (errorMsg1.get(0).equals("uomTypeName") && errorMsg2.get(0).equals(expectMessage)) {
					...
					test.fail("Expected error message is not getting received as expected in response");
					ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "",
							Wsstatus, "" + Wscode, responsestr, "Fail", "uomTypeNm" + expectMessage);
					Assert.fail("Test Failed");
				}
			} else {
				if (Wscode != 400) {
				...
				ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "", Wsstatus,
						"" + Wscode, responsestr, "Fail", "uomTypeNm" + expectMessage);
				Assert.fail("Test Failed");
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void TC_06() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.uomTypePostRequestWithoutUomTypeCd(userId, uomTypeCode, uomTypeName,
					uomTypeDesc);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			logger.info("Request Created");
	...
	public void TC_07() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.uomTypePostRequestWithoutUomTypeNm(userId, uomTypeCode, uomTypeName,
					uomTypeDesc);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			logger.info("Request Created");
	...
	public void TC_08() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.uomTypePostRequestWithoutUomTypeDesc(userId, uomTypeCode, uomTypeName,
					uomTypeDesc);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			logger.info("Input Request created");
		...
		try {
			...
			if (Wscode == 400 && meta != null && actualRespVersionNum.equalsIgnoreCase(actuatorcommandversion)) {
				...
				if (errorMsg1.get(0).equals("uomTypeDescription") && errorMsg2.get(0).equals(expectMessage)) {
					...
					test.fail("Expected error message is not getting received as expected in response");
					ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "",
							Wsstatus, "" + Wscode, responsestr, "Fail", "uomTypeDesc" + expectMessage);
					Assert.fail("Test Failed");
				}
			} else {
				if (Wscode != 400) {
				...
				ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "", Wsstatus,
						"" + Wscode, responsestr, "Fail", "uomTypeDesc" + expectMessage);
				Assert.fail("Test Failed");
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void TC_14() {
		...
		try {
			...
			if (Wscode == 404) {
				...
				ValidationFields.timestampValidation(js, res);   ValidationFields.transactionIdValidation(js, res);
				// ***error message validation
				String expectMessage = resMsgs.invalidUrlMsgUOMTypePUT;
				...
				if (js.getString("data.uomTypeCode") != null) {
					...
					if (internalMsg.equals(expectMessage) && uomTypeCode1.equals(uomTypeCode)) {
						...
						String[] inputFieldNames = { "Input_UserName: ", "Input_uomTypeCode: ", "Input_uomTypeName: ",
								"Input_uomTypeDescription: ", "Input_LastUpdateUserName: " };
						writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
						String[] dbFieldNames = { "DB_UserName: ", "DB_uomTypeCode: ", "DB_uomTypeName: ",
								"DB_uomTypeDescription: ", "DB_LastUpdateUserName: " };
						writableDB_Fields = Miscellaneous.geoDBFieldNames(getResultDB, dbFieldNames);
						test.info("Input Data Values:");
						test.info(writableInputFields.replaceAll("\n", "<br />"));
						test.info("DB Data Values:");
		...
		try {
			...
			if (Wscode == 404 && meta != null && actualRespVersionNum.equalsIgnoreCase(actuatorcommandversion)) {
				...
				if (errorMsg2.get(0).equals(expectMessage)) {
					...
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when the invalid uomTypeCd (not exist in DB) is passed in JSON ");
					ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted,
							writableInputFields, "NA", Wsstatus, "" + Wscode, responsestr1, "Pass", "");
					test.log(Status.PASS, MarkupHelper.createLabel("Test Passed", ExtentColor.GREEN));
				} else {
					logger.error(
							"Expected error message is not getting received in response when the invalid uomTypeCd (not exist in DB) is passed in JSON ");
					logger.info("Execution is completed for Failed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.fail("Expected error message is not getting received as expected in response");
					ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "",
	...
	public void TC_18() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***get end point url
			String getEndPoinUrl = RetrieveEndPoints.getEndPointUrl("geoPost", fileName, level + ".uomType.put");
			test.info("URL: " + getEndPoinUrl);
			// ***send the data to create request and get request
			String payload = PostMethod.uomTypePostRequestMissingComma(userId, uomTypeCode, uomTypeName, uomTypeDesc);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			test.info(reqFormatted.replaceAll("\n", "<br />"));
	...
	public void TC_21() {
		...
		try {
			...
			if (getJMSResult != null) {
				...
				String uomTypeDesc = getJMSResult.getJSONObject("data").getString("uomTypeDescription");
				...
				if (uomTypeCd != null) {
					...
					if (testResult) {
						// ***write result to excel
						String[] responseDbFieldValues = { uomTypeCd, getResultDB.get(0), uomTypeNm, getResultDB.get(1),
								uomTypeDesc, getResultDB.get(2) };
						String[] responseDbFieldNames = { "Response_uomTypeCode: ", "DB_uomTypeCode: ",
								"Response_uomTypeNm: ", "DB_uomTypeName: ", "Response_uomTypeDesc: ",
						...
						String[] responseDbFieldValues = { uomTypeCd, getResultDB.get(0), uomTypeNm, getResultDB.get(1),
								uomTypeDesc, getResultDB.get(2) };
						String[] responseDbFieldNames = { "Response_uomTypeCode: ", "DB_uomTypeCode: ",
								"Response_uomTypeNm: ", "DB_uomTypeName: ", "Response_uomTypeDescription: ",
								"DB_uomTypeDescription: " };
						writableResult = Miscellaneous.geoFieldInputNames(responseDbFieldValues, responseDbFieldNames);

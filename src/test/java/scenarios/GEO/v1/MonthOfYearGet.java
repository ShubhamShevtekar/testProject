...
public class MonthOfYearGet extends Reporting {
	...
	public void TC_01() {
		...
		try {
			...
			// ***get end point url
			String getEndPoinUrl = RetrieveEndPoints.getEndPointUrl("geoGet", fileName, level + ".monthOfYear.get");
			...
			if (Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS") && meta != null
				...
				if (getResultDB.size() == responseRows.size() * fields.size()) {
					...
					for (int i = 0; i < responseRows.size(); i++) {
						if (StringUtils.isBlank(js.getString("data[" + i + "].monthOfYearNumber"))) {
							getResponseRows.add("");
						} else {
							getResponseRows.add(js.getString("data[" + i + "].monthOfYearNumber"));
						}
						if (StringUtils.isBlank(js.getString("data[" + i + "].monthOfYearShortName"))) {
							getResponseRows.add("");
						} else {
							getResponseRows.add(js.getString("data[" + i + "].monthOfYearShortName"));
						}
					}
	...
	public void TC_02() {
		...
		try {
			...
			if (Wscode == 404) {
				...
				String expectMessage = resMsgs.invalidUrlMoyGet;

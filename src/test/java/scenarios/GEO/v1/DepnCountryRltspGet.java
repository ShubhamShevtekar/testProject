...
public class DepnCountryRltspGet extends Reporting {
	...
	public void TC_01() {
		...
		try {
			...
			if (Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS") && meta != null
				...
				List<String> getResultDB = DbConnect.getResultSetFor(depnCntryRltspGetQuery, fields, fileName,
						testCaseID);
			...
			if (responseRows.size() == 0) {
				...
				if (Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS")
					...
					if (internalMsg.equals(expectMessage)) {
						...
						logger.info(
								"Expected error message is getting received in response when passing the invalid dependentCountryCd in URI");
	...
	public void TC_03() {
		...
		try {
			...
			String actualRespVersionNum = js.getString("meta.version");
			if (Wscode == 200 && actualRespVersionNum.equalsIgnoreCase(actuatorQueryVersion)) {
				logger.info("Response status code 200 validation passed: " + Wscode);
				test.pass("Response status code 200 validation passed: " + Wscode);
				...
				if (internalMsg.equals(expectMessage)) {
					logger.info(
							"Expected error message is getting received in response when passing the invalid format of dependentCountryCd in URI");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when passing the invalid format of dependentCountryCd in URI");
	...
	public void TC_04() {
		...
		try {
			...
			if (Wsstatus.equals("404")) {
				...
				if (internalMsg.equals(expectMessage)) {
					logger.info(
							"Expected error message is getting received in response when dependentCountryCd not passing in URI");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when dependentCountryCd not passing in URI");
	...
	// newly added tc here
			...
			public void TC_07() {
				...
				try {
					...
					test.pass("URI passed: "+getEndPoinUrl);
					//***send request and get response
					Response res = GetResponse.sendRequestGet(tokenValues[1], token, getEndPoinUrl, fileName, testCaseID);
					String responsestr=res.asString();
					String responsestr1 = Miscellaneous.jsonFormat(responsestr);
					test.info("Response Recieved:");
					test.info(responsestr1.replaceAll("\n", "<br />"));

...
public class GeoTypePut extends Reporting {
	...
	String TestCaseDescription, scenarioType, userId, geoTypeName, geoTypeId;
	...
	static Logger logger = Logger.getLogger(GeoTypePut.class);
			...
			if (Wscode == 200 && meta != null && Wsstatus.equalsIgnoreCase("SUCCESS")
				...
				if (js.getString("data.geopoliticalTypeId") != null) {
					...
					if (internalMsg.equals(expectMessage) && geoTypeId1.equals(geoTypeId)) {
						...
						test.fail(
								"Success message with Geopolitical ID is not getting received as expected in response");
					...
					String[] inputFieldNames = { "Input_GeoTypeName: ", "Input_GeoTyepId: ",
		...
		try {
			...
			if (Wscode == 400 && meta != null && actualRespVersionNum.equalsIgnoreCase(actuatorcommandversion)) {
				...
				if (errorMgs2.get(0).equals(expectMessage)
						|| errorMgs2.get(1).equals(expectMessage) && errorMgs1.get(0).equals("userName")
						|| errorMgs1.get(1).equals("geopoliticalTypeName")) {
					test.pass("Expected error messages is getting received in response");
					// ***write result to excel
					String[] inputFieldValues = { userId, geoTypeName };
					String[] inputFieldNames = { "Input_UserName: ", "Input_GeoTypeName: " };
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					logger.info(
							"Expected error message is getting received in response when more than one attribute is  empty/ null");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when more than one attribute is  empty/ null");
					...
					ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "",
							...
							"userName and geopoliticalTypeName " + expectMessage);
					Assert.fail("Test Failed");
				}
			} else {
				if (Wscode != 400) {
				...
				ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "", Wsstatus,
						"" + Wscode, responsestr1, "Fail", "userName and geopoliticalTypeName " + expectMessage);
				Assert.fail("Test Failed");
		...
		try {
			...
			if (Wscode == 404 && meta != null && actualRespVersionNum.equalsIgnoreCase(actuatorcommandversion)) {
				...
				if (errorMgs2.get(0).equals(expectMessage) && errorMgs1.get(0).equals("Error")) {
					...
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					logger.info(
							"Expected error message is getting received in response when invalid geopoliticalTypeId is passed in URI");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when invalid geopoliticalTypeId is passed in URI");
	...
	public void TC_11() {
		...
		try {
			...
			if (Wscode == 404 && meta != null && actualRespVersionNum.equalsIgnoreCase(actuatorcommandversion)) {
				...
				ValidationFields.transactionIdValidation(js, res);
				// ***error message validation
				if (errorMgs2.get(0) == null && errorMgs1.get(0).equals("Error")) {
					test.pass("Expected error messages is getting received in response");
					// ***write result to excel
					String[] inputFieldValues = { userId, geoTypeName };
					String[] inputFieldNames = { "Input_UserName: ", "Input_GeoTypeName: " };
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					logger.info(
							"Expected error message is getting received in response when alphabets are passed as geopoliticalTypeId value in URI");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response  when alphabets are passed as geopoliticalTypeId value in URI");
	...
	public void TC_12() {
		...
		try {
			...
			int Wscode = res.statusCode();
			if (Wscode == 500) {
				logger.info("Response status code 500 validation passed: " + Wscode);
				test.pass("Response status code 500 validation passed: " + Wscode);
				// ***error message validation
				...
				if (internalMsg.equals(expectMessage)) {
					...
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					logger.info(
							"Expected error message is getting received in response when special characters are passed as geopoliticalTypeId value in URI");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when special characters are passed as geopoliticalTypeId value in URI");
				...
				logger.error("Response status code 500 validation failed: " + Wscode);
				logger.error("Execution is completed for Failed Test Case No. " + testCaseID);
				logger.error("------------------------------------------------------------------");
				test.fail("Response status code 500 validation failed: " + Wscode);
				ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "", Wsstatus,
						"" + Wscode, responsestr, "Fail", internalMsg);
				Assert.fail("Test Failed");
			}
	...
	public void TC_13() {
		...
		try {
			...
			test.info(reqFormatted.replaceAll("\n", "<br />"));
			// ***get end point url
			String getEndPoinUrl = RetrieveEndPoints.getEndPointUrl("geoPost", fileName, level + ".geoType.put");
			// ***send request and get response
			...
			if (Wscode == 401) {
				...
				if (errorMsg1.equals("Security Error") && errorMsg2.contains(expectMessage)) {
					...
					test.fail("Expected error message is not getting received in response");
					ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "",
							Wsstatus, "" + Wscode, responsestr, "Fail", "geoTypeName" + expectMessage);
					Assert.fail("Test Failed");
				}
			} else {
				...
				ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "", Wsstatus,
						"" + Wscode, responsestr, "Fail", "geoTypeName" + expectMessage);
				Assert.fail("Test Failed");
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void TC_15() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.geoTypePostRequestMissingComma(userId, geoTypeName);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			test.info(reqFormatted.replaceAll("\n", "<br />"));
			// ***get end point url
	...
	public void TC_16() {
		...
		try {
			...
			/*
			 * Response res = GetResponse.sendRequestGet(payload,

...
public class LanguagePut extends Reporting {
	...
	String countryCode, localeScriptCode, engLanguageName, nativeScriptCode, localeCd, localeCd1, countryCode1;;
	...
	public void TC_01() {
		...
		try {
			...
			if (Wscode == 200 && meta != null && Wsstatus.equalsIgnoreCase("SUCCESS")
				...
				// Query1
				String langPutQuery = query.langPostQuery(languageCode);
				// ***get the fields needs to be validate in DB
				List<String> fields = ValidationFields.langPutNewDbFields();
				// ***get the result from DB
				List<String> getResultDB1 = DbConnect.getResultSetFor(langPutQuery, fields, fileName, testCaseID);
				...
				if (js.getString("data.languageCode") != null) {
					...
					if (internalMsg.equals(expectMessage)) {
						...
						String[] inputFieldNames = { "Input_languageCode: ", "Input_nativeScriptLanguageName: ",
								"Input_nativeScriptCode: ", "Input_languageName: ", "Input_LastUpdateUserName: " };
			...
			if (Wscode != 200) {
				...
				if (Wscode == 400 && meta != null && actualRespVersionNum.equalsIgnoreCase(actuatorcommandversion)) {
					...
					if (errorMsg1.get(0).equals("effectiveDate") && errorMsg2.get(0).equals(expectMessage)) {
						...
						logger.info(
								"Expected error message is getting received in response  when the Locale-effectiveDate is passed as empty/null in JSON");
						logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
						logger.info("------------------------------------------------------------------");
						test.pass(
								"Expected error message is getting received in response  when the Locale-effectiveDate is passed as empty/null in JSON");
	...
	public void TC_39() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.langPostNewWithoutEffectiveDateRequest(userId, languageCode,
					nativeScriptLanguageName, nativeScriptCode, languageName, localeCd, countryCode, localesScriptCd,
					cldrVersionDate, cldrVersionNumber, dateFullFormatDescription, dateLongFormatDescription,
					dateMediumFormatDescription, dateShortFormatDescription, localesExpirationDate, translatedDOWs,
					translatedMOYs);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			test.info(reqFormatted.replaceAll("\n", "<br />"));
			...
			if (Wscode != 200) {
				...
				if (Wscode == 400 && meta != null && actualRespVersionNum.equalsIgnoreCase(actuatorcommandversion)) {
					...
					if (errorMsg1.get(0).equals("effectiveDate") && errorMsg2.get(0).equals(expectMessage)) {
						...
						logger.info(
								"Expected error message is getting received in response when the Locale-effectiveDate attribute not passed  in JSON request");
						logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
						logger.info("------------------------------------------------------------------");
						test.pass(
								"Expected error message is getting received in response when the Locale-effectiveDate attribute not passed  in JSON request");
	...
	public void TC_40() {
		...
		try {
			...
			String payload = PostMethod.langPostNewWithoutExpirationDateRequest(userId, languageCode,
					nativeScriptLanguageName, nativeScriptCode, languageName, localeCd, countryCode, localesScriptCd,
					cldrVersionDate, cldrVersionNumber, dateFullFormatDescription, dateLongFormatDescription,
					dateMediumFormatDescription, dateShortFormatDescription, localesEffectiveDate, translatedDOWs,
					translatedMOYs);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			test.info(reqFormatted.replaceAll("\n", "<br />"));
	...
	public boolean auditLanguageDBValidation(String testCaseID, Response res)
		...
		// Audit Query1
		String langPutAuditQuery = query.langPostAuditQuery(languageCode);
		List<String> auditFields = ValidationFields.langPutNewAuditDbFields();
		List<String> getAuditResultDB1 = DbConnect.getResultSetFor(langPutAuditQuery, auditFields, fileName,
				testCaseID);
		...
		inputAuditFieldValues[1] = nativeScriptLanguageName;
		inputAuditFieldValues[2] = nativeScriptCode;
		inputAuditFieldValues[3] = languageName;
		...
		String[] inputAuditFieldNames = { "Input_languageCode: ", "Input_nativeScriptLanguageName: ",
				"Input_nativeScriptCode: ", "Input_languageName: ", "Input_LastUpdateUserName: ",
		...
		String[] auditDBFieldNames = { "DB_languageCode: ", "DB_nativeScriptLanguageName: ", "DB_nativeScriptCode: ",
				"DB_languageName: ", "DB_LastUpdateUserName: ", "DB_RevisionTypeCd: " };
	...
	public boolean auditlocaleDBValidation(String testCaseID, Response res)
		...
		String[] inputFieldValues = { localeCd, countryCode, localesScriptCd, cldrVersionDate, cldrVersionNumber,
				dateFullFormatDescription, dateLongFormatDescription, dateMediumFormatDescription,
				dateShortFormatDescription, localesEffectiveDate, localesExpirationDate, "1" };
		List<String> resFields = ValidationFields.langResponseFileds(res);

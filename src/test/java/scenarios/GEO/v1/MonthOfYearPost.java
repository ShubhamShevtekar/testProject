...
public class MonthOfYearPost extends Reporting {
			...
			if (Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS") && meta != null 
				...
				if (js.getString("data.monthOfYearNumber") != null) {
					...
					if (internalMsg.equals(expectMessage)) {
						...
						String[] dbFieldNames = { "DB_UserName: ", "DB_monthOfYearNumber: ",
								"DB_monthOfYearShortName: ", "DB_LastUpdateUserName: " };
		...
		try {
			...
			if (Wscode == 400 && meta != null 
				...
				if (errorMsg1.get(0).equals("monthOfYearNumber") && errorMsg2.get(0).equals(expectMessage)) {
					...
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					logger.info(
							"Expected error message is getting received in response when the monthOfyearNumber is more than 38 characters length in JSON");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when the monthOfyearNumber is more than 38 characters length in JSON");
	...
	public void TC_11() {
		...
		try {
			...
			List<String> errorMsg1 = new ArrayList<>();
			List<String> errorMsg2 = new ArrayList<>();
			String expectMessage = resMsgs.invalidUrlMsgMOYPost;
			for (int i = 0; i < errorMsgLength; i++) {
				errorMsg1.add(js.getString("errors[" + i + "].fieldName"));
				errorMsg2.add(js.getString("errors[" + i + "].message"));
			}
		...
		try {
			...
			if (Wscode == 400 && meta != null 
				...
				if (errorMsg1.get(0).equals("NA") && errorMsg2.get(0).equals(expectMessage)) {
					...
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					logger.info(
							"Expected error message is getting received in response when the user tried to process the same JSON request again");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when the user tried to process the same JSON request again");
		...
		try {
			...
			if (Wscode == 400 && meta != null 
				...
				if (errorMsg1.get(0).equals("monthOfYearShortName") && errorMsg2.get(0).equals(expectMessage)) {
					...
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					logger.info(
							"Expected error message is getting received in response when the null value is passed in JSON for monthOfyearShortName");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when the null value is passed in JSON for monthOfyearShortName");

...
public class DayOfWeekGraphQL extends Reporting {
	...
	public void TC_01() {
		...
		try {
			...
			String payload = "{\"query\":\"{  daysOfWeek {      dayOfWeekNumber      dayOfWeekFullName      dayOfWeekShortName      }}\",\"variables\":null,\"operationName\":null}";
	...
	public void TC_02() {
		...
		try {
			...
			// ** without parameter
			String payload = "{\"query\":\"{  daysOfWeek {      dayOfWeekNumbe      dayOfWeekFullName      dayOfWeekShortName      }}\",\"variables\":null,\"operationName\":null}";
			test.info("GraphQL Request:");
			test.info(Miscellaneous.jsonFormat(payload));
			// ***send request and get response
			Response res = GetResponse.sendGraphQLRequest(payload, tokenValues[0], token, getEndPoinUrl, fileName,
			...
			if (Wscode == 400 && meta != null && actualRespVersionNum.equalsIgnoreCase(actuatorGraphQLversion)) {
				...
				if (errorMsg1.get(0).equals("ValidationError") && errorMsg2.get(0).equals(expectMessage)) {
					logger.info(
							"Expected error message is getting received in response when sending the invalid dayOfWeekNumber atrribute in request body");
					...
					ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, payload, "NA", "NA",
							Wsstatus, "" + Wscode, responsestr, "Fail", "ValidationError " + expectMessage);
					Assert.fail("Test Failed");
				}
			} else {
				if (Wscode != 400) {
...
//			test.info(responsestr1.replaceAll("\n", "<br />"));
	...
	public void TC_03() {
		...
		try {
			...
			// ** without parameter
			String payload = "{\"query\":\"{  daysOfWeek {      dayOfWeekNumbe      dayOfWeekFullNam      dayOfWeekShortNam      }}\",\"variables\":null,\"operationName\":null}";
			test.info("GraphQL Request:");
			test.info(Miscellaneous.jsonFormat(payload));
			// ***send request and get response
			Response res = GetResponse.sendGraphQLRequest(payload, tokenValues[0], token, getEndPoinUrl, fileName,
			...
			if (Wscode == 400 && meta != null && actualRespVersionNum.equalsIgnoreCase(actuatorGraphQLversion)) {
				...
				if ((errorMsg1.get(0).equals("ValidationError") && errorMsg2.get(0).equals(expectMessage))
					...
					logger.info(
							"Expected error message is getting received in response when sending the multiple invalid attribute in request body");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when sending the multiple invalid attribute in request body");
					...
					ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, payload, "NA", "NA",
							Wsstatus, "" + Wscode, responsestr, "Fail", "ValidationError " + expectMessage
									+ " ValidationError " + expectMessage1 + " ValidationError " + expectMessage2);
					Assert.fail("Test Failed");
				}
			} else {
				if (Wscode != 400) {

...
public class AddressLabelsPut extends Reporting {
	...
	static Logger logger = Logger.getLogger(AddressLabelsPut.class);
	...
	public void TC_01() {
		...
		try {
			...
			if (Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS") && meta != null
				...
				// ***get the DB query
				String addrLabelPutQuery = query.addressLabelPutNewQuery(addrLabelResponseId, localCd,
						formatEffectiveDate, formatExpirationDate);
				// ***get the fields needs to be validate in DB
				List<String> fields = ValidationFields.addressLabelsPUTNewMethodDbFields();
				// ***get the result from DB
				List<String> getResultDB = DbConnect.getResultSetFor(addrLabelPutQuery, fields, fileName, testCaseID);
					...
					if (internalMsg.equals(expectMessage)) {
						...
						if (testResult) {
							...
							ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted,
									...
									"");
							String addrLabelPutQueryAudit = query.addressLabelPutNewQueryAudit(addrLabelResponseId,
									localCd);
							// ***get the fields needs to be validate in DB
							List<String> fieldsAudit = ValidationFields.addressLabelsAuditPutNewMethodDbFields();
							// ***get the result from DB
	...
	public void TC_13() {
		...
		try {
			...
			String expectMessage = resMsgs.lengthExceeds80Char1;
			int Wscode = res.statusCode();
			if (Wscode == 400 && meta != null && actualRespVersionNum.equalsIgnoreCase(actuatorcommandversion)) {
				logger.info("Response status code 400 validation passed: " + Wscode);
				test.pass("Response status code 400 validation passed: " + Wscode);
	...
	public void TC_22() {
		...
		try {
			...
			if (Wscode == 400 && meta != null && actualRespVersionNum.equalsIgnoreCase(actuatorcommandversion)) {
				...
				ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "", Wsstatus,
						"" + Wscode, responsestr, "Fail", "effectiveDates" + expectMessage);
				Assert.fail("Test Failed");
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void TC_23() {
		...
		try {
			...
			if (Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS") && meta != null
				...
				// ***get the DB query
				String addrLabelPutQuery = query.addressLabelPutLangNewQuery(addrLabelResponseId, localCd, languageCode,

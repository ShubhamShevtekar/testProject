					...
					for (int j = 0; j < getResultDB.size(); j = j + fields.size()) {
						if (getResultDB.get(j).toString().equals(getResponseRows.get(j).toString())
							...
							String[] responseDbFieldNames = { "Response_uomTypeCode: ", "DB_uomTypeCode: ",
									"Response_uomTypeName: ", "DB_uomTypeName: ", "Response_uomTypeDescription: ",
			...
			if (responseRows.size() == 0) {
				...
				if (Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS")
					...
					if (internalMsg.equals(expectMessage)) {
						...
						logger.info(
								"Expected error message is getting received in response when passing the invalid uomTypeCode in URI");
	...
	public void TC_04() {
		...
		try {
			...
			if (Wscode == 404) {
				...
				ValidationFields.timestampValidation(js, res);   ValidationFields.transactionIdValidation(js, res); 
				// ***error message validation
				String expectMessage = resMsgs.invalidUrlUOMTypeGet;

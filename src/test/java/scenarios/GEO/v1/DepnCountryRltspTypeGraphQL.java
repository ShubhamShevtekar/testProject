				...
				if (getResultDB.size() == responseRows.size() * fields.size()) {
					...
					for (int i = 0; i < responseRows.size(); i++) {
						if (StringUtils.isBlank(js.getString(
								"data.dependentCountryRelationshipTypes.dependentRelationshipId[" + i + "]"))) {
							getResponseRows.add("");
						} else {
						...
						if (StringUtils.isBlank(
								js.getString("data.dependentCountryRelationshipTypes.dependentRelationshipDescription["
										+ i + "]"))) {
							getResponseRows.add("");
						} else {
							getResponseRows.add(js.getString(
									"data.dependentCountryRelationshipTypes.dependentRelationshipDescription[" + i
											+ "]"));
						}
					}

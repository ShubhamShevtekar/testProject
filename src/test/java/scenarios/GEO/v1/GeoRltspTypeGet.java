...
public class GeoRltspTypeGet extends Reporting{
	...
	String TestCaseDescription, scenarioType, geopoliticalRelationshipTypeCode;
	...
	{	
		...
		try {
			...
			//***get end point url
			String getEndPoinUrl = RetrieveEndPoints.getEndPointUrl("geoGet", fileName, level+".geoRsType.get");
	        ...
	        if(Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS") && actualRespVersionNum.equalsIgnoreCase("1.0.0"))
	        {
	    		...
	    		if(getResultDB.size() == responseRows.size()*fields.size())
	    		{
		        	...
		        	{
		        		if(StringUtils.isBlank(js.getString("data["+i+"].geopoliticalRelationshipTypeCode")))
		        		{
		        			getResponseRows.add("");
		        		}else {
		        			getResponseRows.add(js.getString("data["+i+"].geopoliticalRelationshipTypeCode"));
		        		}
		        		if(StringUtils.isBlank(js.getString("data["+i+"].geopoliticalRelationshipTypeDescription")))
		        		{
		        			getResponseRows.add("");
		        		}else {
		        			getResponseRows.add(js.getString("data["+i+"].geopoliticalRelationshipTypeDescription"));
		        		}
		        	}
		        	...
		        	for(int j=0; j<getResultDB.size(); j=j+fields.size())
		        	{
		        		if(getResultDB.get(j).toString().equals(getResponseRows.get(j).toString()) && getResultDB.get(j+1).toString().equals(getResponseRows.get(j+1).toString()))
		        		{
		        			...
		        			String[] responseDbFieldNames = {"Response_geopoliticalRelationshipTypeCode: ", "DB_geopoligeopoliticalRelationshipTypeCodeeCode: ", 
		        					"Response_geopoliticalRelationshipTypeDescription: ", "DB_geopoliticalRelationshipTypeDescription: "};
		        			...
		        			String[] responseDbFieldValues = {getResponseRows.get(j).toString(), getResultDB.get(j).toString(), 
		        					getResponseRows.get(j+1).toString(), getResultDB.get(j+1).toString()};
		        			String[] responseDbFieldNames = {"Response_geopoliticalRelationshipTypeCode: ", "DB_geopoliticalRelationshipTypeCode: ", 
		        					"Response_geopoliticalRelationshipTypeDescription: ", "DB_geopoliticalRelationshipTypeDescription: "};
		        			writableResult = Miscellaneous.geoFieldInputNames(responseDbFieldValues, responseDbFieldNames);
		        			test.info("Record "+z+" Validation:");
	...
	{	
		...
		try {
			...
			//***get end point url
			String getEndPoinUrl = RetrieveEndPoints.getEndPointUrl("geoGet", fileName, level+".geoRsType.get");
			getEndPoinUrl = getEndPoinUrl+geopoliticalRelationshipTypeCode;
			logger.info("URI passed: "+getEndPoinUrl);
        	test.pass("URI passed: "+getEndPoinUrl);
			//***send request and get response
			Response res = GetResponse.sendRequestGet(tokenValues[0], token, getEndPoinUrl, fileName, testCaseID);
	        ...
	        if(Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS") && actualRespVersionNum.equalsIgnoreCase("1.0.0"))
	        {
	        	...
	        	ValidationFields.timestampValidation(js, res);   ValidationFields.transactionIdValidation(js, res);
	        	//***get the DB query
	    		String geoRsTypeGetQuery = query.geoRsTypePostQuery(geopoliticalRelationshipTypeCode);
	    		//***get the fields needs to be validate in DB
	    		List<String> fields = ValidationFields.geoRsTypeGetMethodDbFields();
	    		//***get the result from DB
	    		List<String> getResultDB = DbConnect.getResultSetFor(geoRsTypeGetQuery, fields, fileName, testCaseID);
		...
		try {
	        ...
	        if(responseRows.size()==0)
	        {
		        ...
		        if(Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS") && actualRespVersionNum.equalsIgnoreCase("1.0.0"))
		        {
					...
					{
						String[] inputFieldValues = {geopoliticalRelationshipTypeCode};
						String[] inputFieldNames = {"Input_geopoliticalRelationshipTypeCode: "};
		        		writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
		        		logger.info("Expected error message is getting received in response when passing the invalid geopoliticalRelationshipTypeCode in URI");
		        		logger.info("Execution is completed for Passed Test Case No. "+testCaseID);
		    			logger.info("------------------------------------------------------------------");
		        		test.pass("Expected error message is getting received in response when passing the invalid geopoliticalRelationshipTypeCode in URI");
	...
	public void TC_05() {
		...
		try {
			...
			Response res = GetResponse.sendRequestPost("", tokenValues[0], token, getEndPoinUrl, fileName,
					testCaseID);
			...
			if (Wscode == 405 && actualRespVersionNum.equalsIgnoreCase(actuatorQueryVersion)) {
				...
				if (errorMsg1.get(0).equals("Error") && errorMsg2.get(0).equals(expectMessage)) {
					logger.info("Expected error message is getting received in response when use GET url but selecting POST method.");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass("Expected error message is getting received in response when use GET url but selecting POST method.");
		...
		try {
			...
			if (Wscode == 401 && actualRespVersionNum.equalsIgnoreCase(actuatorQueryVersion)) {
				...
				if (errorMsg1.get(0).equals("NA") && errorMsg2.get(0).equals(expectMessage)) {
					logger.info("Expected error message is getting received in response when missing HTTP Header X-CSR-SECURITY_TOKEN. ");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass("Expected error message is getting received in response when missing HTTP Header X-CSR-SECURITY_TOKEN. ");
					ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, "NA", "", "NA", Wsstatus,
							"" + Wscode, responsestr1, "Pass", "");
					test.log(Status.PASS, MarkupHelper.createLabel("test status", ExtentColor.GREEN));
				} else {
	...
	public void testDataFields(String scenarioName, String testCaseId)
	{
		...
		geopoliticalRelationshipTypeCode = inputData1.get(testCaseId).get("relationshipTypeCode");
	}

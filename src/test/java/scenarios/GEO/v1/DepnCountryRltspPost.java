...
public class DepnCountryRltspPost extends Reporting {
	...
	String TestCaseDescription, scenarioType, userId, depnCntryRltspDesc;// ,
																			// token;
	...
	public void TC_01() {
		...
		try {
			...
			// ***get end point url
			String getEndPoinUrl = RetrieveEndPoints.getEndPointUrl("geoPost", fileName,
					level + ".depnCntryRltsp.post");
			...
			if (Wscode == 200 && Wsstatus.equalsIgnoreCase("Success") && meta != null && (!meta.contains("timestamp"))
				...
				if (js.getString("data.dependentRelationshipId") != null) {
					...
					String[] inputFieldNames = { "Input_UserName: ", "Response_DepnCntryRltspId: ",
							"Input_DepnCntryRltspDesc: ", "Input_LastUpdateUserName: " };
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					String[] dbFieldNames = { "DB_UserName: ", "DB_DepnCntryRltspId: ", "DB_DepnCntryRltspDesc: ",
	...
	public void TC_08() {
		...
		try {
			...
			if (Wscode == 404) {
				...
				test.pass("Response status code 400 validation passed: " + Wscode);
				// ***error message validation
				String expectMessage = resMsgs.invalidUrldentcountryMsgstPost1;
				if (internalMsg.equals(expectMessage)) {
		...
		try {
			...
			if (Wscode == 400 && meta != null && (!meta.contains("timestamp"))
				...
				if (errorMsg1.get(0).equals("NA") && errorMsg2.get(0).equals(expectMessage)) {
					...
					logger.info(
							"Expected error message is getting received in response when the user tried to process the same dependentRelationshipDescription again");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
	...
	// newly added tc's for new changes POST
	@Test(priority = 2)
	public void TC_10() {
		String testCaseID = new Object() {
		}.getClass().getEnclosingMethod().getName();

...
public class GeoRltspTypePost extends Reporting {
		...
		try {
			...
			if (Wscode == 200 && meta != null && Wsstatus.equalsIgnoreCase("SUCCESS")
				...
				if (js.getString("data.geopoliticalRelationshipTypeCode") != null) {
					...
					String[] dbFieldNames = { "DB_UserName: ", "DB_GeoRltspTypeCode: ", "DB_GeoRltspTypeDesc: ",
	...
	public void TC_02() {
		...
		// Geopolitical Relationship Type service is not created when the
		// Geopolitical Relationship Type Code is empty in JSON Request.",
		// ExtentColor.PURPLE));
		// ***get test case ID with method name
		try {
			// ***get the test data from sheet
	...
	public void TC_07() {
		...
		try {
			...
			String Wsstatus = res.getStatusLine();
			int Wscode = res.statusCode();
			String expectMessage = resMsgs.invalidUrlMsgGeoRlshipTypePost;
			String meta = js.getString("meta");
			String actualRespVersionNum = js.getString("meta.version");
			int errorMsgLength = js.get("errors.size");
			List<String> errorMgs1 = new ArrayList<String>();

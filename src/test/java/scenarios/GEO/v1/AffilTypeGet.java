...
public class AffilTypeGet extends Reporting {
	...
	String TestCaseDescription, scenarioType, affiliationTypeCode;
			...
			if (Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS") && meta != null
				...
				if (getResultDB.size() == responseRows.size() * fields.size()) {
					...
					for (int i = 0; i < responseRows.size(); i++) {
						if (StringUtils.isBlank(js.getString("data[" + i + "].affiliationTypeId"))) {
							getResponseRows.add("");
						} else {
							getResponseRows.add(js.getString("data[" + i + "].affiliationTypeId"));
						}
						if (StringUtils.isBlank(js.getString("data[" + i + "].affiliationTypeCode"))) {
							getResponseRows.add("");
						} else {
							getResponseRows.add(js.getString("data[" + i + "].affiliationTypeCode"));
						}
						if (StringUtils.isBlank(js.getString("data[" + i + "].affiliationTypeName"))) {
							getResponseRows.add("");
						} else {
							getResponseRows.add(js.getString("data[" + i + "].affiliationTypeName"));
						}
					}
	...
	public void TC_02() {
		...
		try {
			...
			// ***get end point url
			String getEndPoinUrl = RetrieveEndPoints.getEndPointUrl("geoGet", fileName, level + ".affilType.get");
			getEndPoinUrl = getEndPoinUrl + affiliationTypeCode;
			logger.info("URI passed: " + getEndPoinUrl);
			test.pass("URI passed: " + getEndPoinUrl);
			// ***send request and get response
			Response res = GetResponse.sendRequestGet(tokenValues[0], token, getEndPoinUrl, fileName, testCaseID);
			...
			if (Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS") && meta != null
				...
				ValidationFields.timestampValidation(js, res);   ValidationFields.transactionIdValidation(js, res);
				// ***get the DB query
				String affilTypeGetQuery = query.affilTypePostQuery(affiliationTypeCode);
				// ***get the fields needs to be validate in DB
				List<String> fields = ValidationFields.affilTypeGetMethodDbFields();
				// ***get the result from DB
				List<String> getResultDB = DbConnect.getResultSetFor(affilTypeGetQuery, fields, fileName, testCaseID);
	...
	public void TC_03() {
		...
		try {
			...
			if (Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS") && meta != null
				...
				if (internalMsg.contains("There are no records for the specified criteria")) {
					test.pass("Expected message received when passed invalid AffilType code");
				} else {
					test.fail("Expected message not received when passed invalid AffilType code");
					// ***get the DB query
					String affilTypeGetQuery = query.affilTypePostQuery(affiliationTypeCode);
					// ***get the fields needs to be validate in DB
					List<String> fields = ValidationFields.affilTypeGetMethodDbFields();
					// ***get the result from DB
					List<String> getResultDB = DbConnect.getResultSetFor(affilTypeGetQuery, fields, fileName,
							testCaseID);
					...
					if (getResultDB.size() == responseRows.size() * fields.size()) {
						...
						for (int j = 0; j < getResultDB.size(); j = j + fields.size()) {
							if (getResultDB.get(j).toString().equals(getResponseRows.get(j).toString())
								...
								String[] responseDbFieldValues = { getResponseRows.get(j).toString(),
										...
										getResultDB.get(j + 1).toString(), getResponseRows.get(j + 2).toString(),
										getResultDB.get(j + 2).toString() };
								String[] responseDbFieldNames = { "Response_affiliationTypeId: ",
										"DB_affiliationTypeId: ", "Response_affiliationTypeCode: ",
										"DB_affiliationTypeCode: ", "Response_affiliationTypeName: ",
						...
						logger.error("Execution is completed for Failed Test Case No. " + testCaseID);
						logger.error("------------------------------------------------------------------");
						test.fail("Total number of records not matching between DB: "
								+ getResultDB.size() / fields.size() + " & Response: " + responseRows.size());
						ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, "", "", "", Wsstatus,
								"" + Wscode, responsestr1, "Fail", internalMsg);
						Assert.fail("Test Failed");
	...
	public void TC_04() {
		...
		try {
			...
			if (Wscode == 404) {
				...
				String expectMessage = resMsgs.invalidUrlAffilTypeGet;
				if (errorMsg1.get(0).equals("Error") && errorMsg2.get(0).equals(expectMessage)) {
					logger.info("Expected error message is getting received in response when passing invalid URI");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass("Expected error message is getting received in response when passing invalid URI");

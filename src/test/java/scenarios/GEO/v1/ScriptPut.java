...
public class ScriptPut extends Reporting {
		...
		try {
			...
			if (Wscode == 200 && meta != null && Wsstatus.equalsIgnoreCase("SUCCESS")
				...
				if (js.getString("data.scriptCode") != null) {
					...
					String expectMessage = resMsgs.scriptPutNewSuccessMsg + scriptCode1;
					if (internalMsg.equals(expectMessage)) {
						...
						String[] inputFieldValues = { scriptCode, scriptName, scriptDesc, effectiveDate, expirationDate,
								userId };
						...
						String[] inputFieldNames = { "Input_ScriptCode: ", "Input_ScriptName: ",
								"Input_ScriptDescription: ", "Input_effectiveDate: ", "Input_expirationDate: ",
								"Input_LastUpdateUserName: " };
						writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
						String[] dbFieldNames = { "DB_ScriptCode: ", "DB_ScriptName: ", "DB_ScriptDescription: ",
						...
						if (testResult) {
							...
							String[] inputAuditFieldValues = { scriptCode, scriptName, scriptDesc, effectiveDate,
							...
							if (testResult) {
								String[] inputFieldNamesAudit = { "Input_ScriptCode: ", "Input_ScriptName: ",
										"Input_ScriptDescription: ", "Input_effectiveDate: ", "Input_expirationDate: ",
								...
								String[] dbFieldNamesAudit = { "DB_ScriptCode: ", "DB_ScriptName: ",
		...
		try {
			...
			if (Wscode == 200 && meta != null && Wsstatus.equalsIgnoreCase("SUCCESS")
				...
				if (js.getString("data.scriptCode") != null) {
					...
					logger.error("scriptCode is not available in response");
					logger.error("Execution is completed for Failed Test Case No. " + testCaseID);
					logger.error("------------------------------------------------------------------");
					test.fail("Script Code is not available in response");
					ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "",
		...
		try {
			...
			if (Wscode == 400 && meta != null && actualRespVersionNum.equalsIgnoreCase(actuatorcommandversion)) {
				...
				if (errorMgs2.get(0).equals(expectMessage) && errorMgs1.get(0).equals("scriptDescription")) {
					...
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					logger.info(
							"Expected error message is getting received in response when the scriptDescription is more than 4000 characters length in JSON");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when the scriptDescription is more than 4000 characters length in JSON");
	...
	public void TC_15() {
		...
		try {
			...
			if (Wscode == 200 && meta != null && Wsstatus.equalsIgnoreCase("SUCCESS")
				...
				// ***removing user name field since we are going to validate
				// only last updated user name
				fields.remove(0);
		...
		try {
			...
			if (Wscode == 404 && meta != null && actualRespVersionNum.equalsIgnoreCase(actuatorcommandversion)) {
				...
				if (errorMgs2.get(0).equals(expectMessage) && errorMgs1.get(0).equals("Error")) {
					logger.info(
							"Expected error message is getting received in response when the invalid scriptCode (not exist in DB) is passed in JSON");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					String[] inputFieldValues = { userId, scriptCode, scriptName, scriptDesc, effectiveDate,
							expirationDate };
	...
	public void TC_18() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.scriptPostRequestMissingComma(userId, scriptCode, scriptName, scriptDesc,
					effectiveDate, expirationDate);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			logger.info("Request Created");
	...
	public void TC_19() {
		...
		try {
			...
			// ***send request and get response
			/*
			 * Response res = GetResponse.sendRequestPut(payload,

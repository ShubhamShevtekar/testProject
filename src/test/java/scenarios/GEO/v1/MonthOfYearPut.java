...
public class MonthOfYearPut extends Reporting {

	String scenarioName = getClass().getSimpleName();
	String TestCaseDescription, scenarioType, userId, monthOfYearNumber, monthOfYearShortName;// ,
																								// token;
	...
	static Logger logger = Logger.getLogger(MonthOfYearPut.class);
	...
	public void TC_01() {
		...
		try {
			...
			// ***get end point url
			String getEndPoinUrl = RetrieveEndPoints.getEndPointUrl("geoPut", fileName, level + ".monthOfYear.put");
			...
			if (Wscode == 200 && meta != null && Wsstatus.equalsIgnoreCase("SUCCESS") && (!meta.contains("timestamp"))
				...
				// ***get the DB query
				String monthOfYearPutQuery = query.monthOfYearPostQuery(monthOfYearNumber);
				...
				List<String> getResultDB = DbConnect.getResultSetFor(monthOfYearPutQuery, fields, fileName, testCaseID);
				if (js.getString("data.monthOfYearNumber") != null) {
					String monthOfYearNumber1 = js.getString("data.monthOfYearNumber");
					// ***success message validation
					String expectMessage = resMsgs.monthOfYearPutSuccessMsg + monthOfYearNumber1;
					if (internalMsg.equals(expectMessage)) {
						logger.info(
								"Success message with Month of Year Number is getting received as expected in response");
						test.pass(
								"Success message with Month of Year Number is getting received as expected in response");
						// ***send the input, response, DB result for validation
						String[] inputFieldValues = { monthOfYearNumber, monthOfYearShortName, userId };
						...
						String[] dbFieldNames = { "DB_monthOfYearNumber: ", "DB_monthOfYearShortName: ",
					...
					logger.error("Month of year number is not available in response");
					logger.error("Execution is completed for Failed Test Case No. " + testCaseID);
					logger.error("------------------------------------------------------------------");
					test.fail("Month of year number is not available in response");
					ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "",
		...
		try {
			...
			if (Wscode == 400 && meta != null && (!meta.contains("timestamp"))
				...
				if (errorMsg1.get(0).equals("monthOfYearNumber") && errorMsg2.get(0).equals(expectMessage)) {
					...
					String[] inputFieldNames = { "Input_UserName: ", "Input_monthOfYearNumber: ",
							"Input_monthOfYearShortName: " };
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					logger.info(
							"Expected error message is getting received in response when sending the blank MonthOfYear Number");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when sending the blank MonthOfYear Number");
		...
		try {
			...
			if (Wscode == 400 && meta != null && (!meta.contains("timestamp"))
				...
				if (errorMsg1.get(0).equals("monthOfYearShortName") && errorMsg2.get(0).equals(expectMessage)) {
					...
					test.fail("Expected error message is not getting received as expected in response");
					ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "",
							Wsstatus, "" + Wscode, responsestr, "Fail", "monthOfYearShortName" + expectMessage);
					Assert.fail("Test Failed");
				}
			} else {
				if (Wscode != 400) {
				...
				ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "", Wsstatus,
						"" + Wscode, responsestr1, "Fail", "monthOfYearShortName" + expectMessage);
				Assert.fail("Test Failed");
			}
		} catch (Exception e) {
			e.printStackTrace();
		...
		try {
			...
			if (Wscode == 400 && meta != null && (!meta.contains("timestamp"))
				...
				if (errorMsg1.get(0).equals("monthOfYearNumber") && errorMsg2.get(0).equals(expectMessage)) {
					...
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					logger.info(
							"Expected error message is getting received in response when the monthOfyearNumber attribute is not passed in JSON");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when the monthOfyearNumber attribute is not passed in JSON");
	...
	public void TC_08() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.monthOfYearPostRequestmonthOfYearNumberAsNumber(userId, monthOfYearNumber,
			...
			List<String> errorMsg1 = new ArrayList<>();
			List<String> errorMsg2 = new ArrayList<>();
			String expectMessage = resMsgs.MoyNumInBetween1and12;
			for (int i = 0; i < errorMsgLength; i++) {
				errorMsg1.add(js.getString("errors[" + i + "].fieldName"));
				errorMsg2.add(js.getString("errors[" + i + "].message"));
			}
		...
		try {
			...
			if (Wscode == 400 && meta != null && (!meta.contains("timestamp"))
				...
				if (errorMsg1.get(0).equals("monthOfYearShortName") && errorMsg2.get(0).equals(expectMessage)) {
					...
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					logger.info(
							"Expected error message is getting received in response when the monthOfyearShortName  is more than 18 characters length in JSON");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when the monthOfyearShortName  is more than 18 characters length in JSON");
				...
				ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "", Wsstatus,
						"" + Wscode, responsestr, "Fail", "monthOfYearShortName" + expectMessage);
				Assert.fail("Test Failed");
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void TC_11() {
		...
		try {
			...
			List<String> errorMsg1 = new ArrayList<>();
			List<String> errorMsg2 = new ArrayList<>();
			String expectMessage = resMsgs.invalidUrlMsgMOYPut;
			for (int i = 0; i < errorMsgLength; i++) {
				errorMsg1.add(js.getString("errors[" + i + "].fieldName"));
				errorMsg2.add(js.getString("errors[" + i + "].message"));
			}
			...
			if (Wscode == 404 && meta != null && actualRespVersionNum.equalsIgnoreCase(actuatorcommandversion)) {
				...
				if (errorMsg1.get(0).equals("Error") && errorMsg2.get(0).equals(expectMessage)) {
					...
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					logger.info(
							"Expected error message is getting received in response when the Invalid URI is Passed");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when the when the Invalid URI is Passed");
					ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted,
							writableInputFields, "NA", Wsstatus, "" + Wscode, responsestr1, "Pass", "");
					test.log(Status.PASS, MarkupHelper.createLabel("test status", ExtentColor.GREEN));
				} else {
		...
		try {
			...
			if (Wscode == 200 && meta != null && Wsstatus.equalsIgnoreCase("SUCCESS") && (!meta.contains("timestamp"))
				...
				fields.remove(0);// ***removing user name field since we are
									...
									// name
				// ***get the result from DB
				List<String> getResultDB = DbConnect.getResultSetFor(monthOfYearPostQuery, fields, fileName,
						testCaseID);
				if (js.getString("data.monthOfYearNumber") != null) {
					String monthOfYearNumber1 = js.getString("data.monthOfYearNumber");
					// ***success message validation
		...
		try {
			...
			if (Wscode == 400 && meta != null && (!meta.contains("timestamp"))
				...
				if (errorMsg1.get(0).equals("monthOfYearNumber") && errorMsg2.get(0).equals(expectMessage)) {
					...
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					logger.info(
							"Expected error message is getting received in response when the null value is passed in JSON for monthOfyearNumber");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when the null value is passed in JSON for monthOfyearNumber");
	...
	public void TC_14() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.monthOfYearPostRequestWithNullMonthOfYearShortName(userId, monthOfYearNumber,
					monthOfYearShortName);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			test.info(reqFormatted.replaceAll("\n", "<br />"));
	...
	public void TC_15() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.monthOfYearPostRequestWithoutUserName(userId, monthOfYearNumber,
					monthOfYearShortName);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			test.info(reqFormatted.replaceAll("\n", "<br />"));
			...
			if (Wscode == 400 && meta != null && (!meta.contains("timestamp"))
				...
				if (errorMsg1.get(0).equals("userName") && errorMsg2.get(0).equals(expectMessage)) {
					...
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					logger.info(
							"Expected error message is getting received in response when the null value is passed in JSON for user name");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when the null value is passed in JSON for user name");
		...
		try {
			...
			if (Wscode == 400 && meta != null && (!meta.contains("timestamp"))
				...
				if (errorMsg1.get(0).equals("monthOfYearNumber") && errorMsg2.get(0).equals(expectMessage)) {
					...
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					logger.info(
							"Expected error message is getting received in response when the invalid monthofyearNumber (not exist in DB) is passed in JSON");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when the invalid monthofyearNumber (not exist in DB) is passed in JSON");
	...
	public void TC_17() {
		...
		try {
			...
			List<String> errorMsg1 = new ArrayList<>();
			List<String> errorMsg2 = new ArrayList<>();
			String expectMessage = resMsgs.jsonParserErrorMsg1;
			for (int i = 0; i < errorMsgLength; i++) {
				errorMsg1.add(js.getString("errors[" + i + "].fieldName"));
				errorMsg2.add(js.getString("errors[" + i + "].message"));
			}
			...
			if (Wscode == 400 && meta != null && (!meta.contains("timestamp"))
				...
				if (errorMsg1.get(0).equals("Error") && errorMsg2.get(0).equals(expectMessage)) {
					...
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					logger.info(
							"Expected error message is getting received in response when the invalid monthofyearNumber (alphabets) is passed in JSONr");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when the invalid monthofyearNumber (alphabets) is passed in JSONr");
		...
		try {
			...
			if (Wscode == 400 && meta != null && (!meta.contains("timestamp"))
				...
				if (errorMsg1.get(0).equals("Error") && errorMsg2.get(0).equals(expectMessage)) {
					...
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					logger.info(
							"Expected error message is getting received in response when the invalid monthofyearNumber (special characters) is passed in JSON");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when the invalid monthofyearNumber (special characters) is passed in JSON");
	...
	public void TC_19() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.monthOfYearPostwitCommaMissingRequest(userId, monthOfYearNumber,
					monthOfYearShortName);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			test.info(reqFormatted.replaceAll("\n", "<br />"));
			// ***get end point url
			String getEndPoinUrl = RetrieveEndPoints.getEndPointUrl("geoPost", fileName, level + ".monthOfYear.put");
			// ***send request and get response
			...
			if (Wscode == 400 && meta != null && (!meta.contains("timestamp"))
				...
				if (errorMsg1.get(0).equals("Error") && errorMsg2.get(0).equals(expectMessage)) {
					...
					String[] inputFieldNames = { "Input_UserName: ", "Input_monthOfYearNumber: ",
							"Input_monthOfYearShortName: ", "Input_LastUpdateUserName: " };
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					logger.info(
		...
		try {
			...
			if (getJMSResult != null) {
				...
				if (monthOfYearNumber != null) {
					...
					String[] responseDbFieldValues = { monthOfYearNumber, getResultDB.get(0), monthOfYearShortName,
							getResultDB.get(1) };

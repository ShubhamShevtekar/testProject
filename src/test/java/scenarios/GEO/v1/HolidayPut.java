...
public class HolidayPut extends Reporting {
	...
	String TestCaseDescription, scenarioType, userId, holidayName, holidayParamText, holidayId;// ,
																								// token;
			...
			if (Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS")
				...
				if (js.getString("data.holidayId") != null) {
					...
					if (internalMsg.equals(expectMessage)) {
						...
						String[] inputFieldNames = { "Response_HolidayId: ", "Input_HolidayName: ",
								"Input_holidayDateParameterText: ", "Input_LastUpdateUserName: " };
						writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
						String[] dbFieldNames = { "DB_HolidayId: ", "DB_HolidayName: ", "DB_holidayDateParameterText: ",
	...
	public void TC_11() {
		...
		try {
			...
			test.info(reqFormatted.replaceAll("\n", "<br />"));
			// ***get end point url
			String getEndPoinUrl = RetrieveEndPoints.getEndPointUrl("geoPost", fileName, level + ".holiday.put");
			...
			if (Wscode == 404) {
				...
				ValidationFields.transactionIdValidation(js, res);
				// ***error message validation
				String expectMessage = resMsgs.invalidUrlMsgHolidayPUT;
		...
		try {
			...
			if (Wscode == 404 && meta != null && actualRespVersionNum.equalsIgnoreCase(actuatorcommandversion)) {
				...
				if (errorMsg1.get(0).equals("Error") && errorMsg2.get(0).equals(expectMessage)) {
					...
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					logger.info(
							"Expected error message is getting received in response when the Invalid holidayId (not exist in DB) is provided");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when the Invalid holidayId (not exist in DB) is provided");
		...
		try {
			...
			if (Wscode == 404 && meta != null && actualRespVersionNum.equalsIgnoreCase(actuatorcommandversion)) {
				...
				if (errorMsg1.get(0).equals("Error") && errorMsg2.get(0) == null) {
					...
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					logger.info(
							"Expected error message is getting received in response when the Invalid holidayId (alphabets) is provided");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when the Invalid holidayId (alphabets) is provided");
		...
		try {
			...
			if (Wscode == 404 && meta != null && actualRespVersionNum.equalsIgnoreCase(actuatorcommandversion)) {
				...
				if (errorMsg1.get(0).equals("Error") && errorMsg2.get(0) == null) {
					...
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					logger.info(
							"Expected error message is getting received in response when the Invalid holidayId (special characters) is provided");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when the Invalid holidayId (special characters) is provided");
	...
	public void TC_16() {
		...
		try {
			...
			test.info("URL: " + getEndPoinUrl);
			// ***send the data to create request and get request
			String payload = PostMethod.holidaPostRequestwithMissingComma(userId, holidayName, holidayParamText);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			test.info(reqFormatted.replaceAll("\n", "<br />"));

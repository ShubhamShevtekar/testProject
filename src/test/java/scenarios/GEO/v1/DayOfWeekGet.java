...
public class DayOfWeekGet extends Reporting{
	...
	static Logger logger = Logger.getLogger(DayOfWeekGet.class);
	...
	public void TC_01()
	{
		...
		try {
			...
			//***get end point url
			String getEndPoinUrl = RetrieveEndPoints.getEndPointUrl("geoGet", fileName, level+".dayOfWeek.get");
	        		...
	        		&& actualRespVersionNum.equalsIgnoreCase(actuatorQueryVersion))
	        {
	    		...
	    		if(getResultDB.size() == responseRows.size()*fields.size())
	    		{
		        	...
		        	{
		        		if(StringUtils.isBlank(js.getString("data["+i+"].dayOfWeekNumber")))
		        		{
		        			getResponseRows.add("");
		        		}else {
		        			getResponseRows.add(js.getString("data["+i+"].dayOfWeekNumber"));
		        		}
		        		if(StringUtils.isBlank(js.getString("data["+i+"].dayOfWeekFullName")))
		        		{
		        			getResponseRows.add("");
		        		}else {
		        			getResponseRows.add(js.getString("data["+i+"].dayOfWeekFullName"));
		        		}
		        		if(StringUtils.isBlank(js.getString("data["+i+"].dayOfWeekShortName")))
		        		{
		        			getResponseRows.add("");
		        		}else {
		        			getResponseRows.add(js.getString("data["+i+"].dayOfWeekShortName"));
		        		}
		        	}
	...
	public void TC_02()
	{
		...
		try {
			...
			if (Wscode == 404) {
				...
				String expectMessage = resMsgs.invalidUrlDoWGet;
	...
	public void TC_03() {
		...
		try {
			...
			// ***get end point url
			String getEndPoinUrl = RetrieveEndPoints.getEndPointUrl("geoGet", fileName, level + ".dayOfWeek.get");
			// getEndPoinUrl = getEndPoinUrl.substring(0, getEndPoinUrl.length()
			// - 2);
			logger.info("URI passed: " + getEndPoinUrl);
			test.pass("URI passed: " + getEndPoinUrl);
			String payload = "abc";

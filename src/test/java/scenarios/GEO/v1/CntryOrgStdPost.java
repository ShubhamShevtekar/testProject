...
public class CntryOrgStdPost extends Reporting {
	...
	String TestCaseDescription, scenarioType, userId, countryCode, countryShortName, countryFullName,
			organizationStandardCode, effectiveDate, expirationDate;// , token;
	...
	static Logger logger = Logger.getLogger(CntryOrgStdPost.class);
	...
	public void TC_01() {
		...
		// test.log(Status.INFO, MarkupHelper.createLabel("Validate that the
		// Script service is successfully created when valid values are passed
		// for all attributes in JSON Request.", ExtentColor.PURPLE));
		...
		try {
			...
			String payload = PostMethod.cntryOrgStdPostRequest(userId, countryCode, countryShortName, countryFullName,
			...
			if (Wscode == 200 && meta != null && Wsstatus.equalsIgnoreCase("SUCCESS") && (!meta.contains("timestamp"))
				...
				String countryOrgStdPostQuery = query.countryOrgStdPostQuery(countryCode, countryShortName,
						countryFullName, organizationStandardCode, formatEffectiveDate, formatExpirationDate);
				// ***get the fields needs to be validate in DB
				List<String> fields = ValidationFields.cntryOrgStdDbFields();
				// ***get the result from DB
				List<String> getResultDB = DbConnect.getResultSetFor(countryOrgStdPostQuery, fields, fileName,
						testCaseID);
				if (js.getString("data.organizationStandardCode") != null
						&& js.getString("data.geopoliticalId") != null) {
					...
					String expectMessage = resMsgs.countryOrgStdPostSuccessMsg + organizationStandardCode1;
					if (internalMsg.equals(expectMessage)) {
						...
						String[] inputFieldValues = { userId, geopId, countryShortName, countryFullName,
								organizationStandardCode, effectiveDate, expirationDate, userId };
						...
						String[] inputFieldNames = { "Input_UserName: ", "Response_GeoId: ", "Input_CountryShortName: ",
								"Input_CountryFullame: ", "Input_organizationStandardCode: ", "Input_effectiveDate: ",
								"Input_expirationDate: ", "Input_LastUpdateUserName: " };
						writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
						String[] dbFieldNames = { "DB_UserName: ", "DB_GeoId: ", "DB_CountryShortName: ",
								"DB_CountryFullame: ", "DB_organizationStandardCode: ", "DB_effectiveDate: ",
						...
						if (testResult) {
							...
							String countryOrgStdPostAuditQuery = query.countryOrgStdPostAuditQuery(countryCode,
									countryShortName, countryFullName, organizationStandardCode,
									formatAuditEffectiveDate, formatAuditExpirationDate);
							// ***get the fields needs to be validate in DB
							List<String> auditFields = ValidationFields.cntryOrgStdAuditDbFields();
							// ***get the result from DB
							List<String> getResultDBAudit = DbConnect.getResultSetFor(countryOrgStdPostAuditQuery,
									auditFields, fileName, testCaseID);
							String[] inputAuditFieldValues = { userId, geopId, countryShortName, countryFullName,
									organizationStandardCode, effectiveDate, expirationDate, userId, "0" };
							testResult = false;
							testResult = TestResultValidation.testValidationWithDB(res, inputAuditFieldValues,
									getResultDBAudit, resFields);

							if (testResult) {
								String[] inputFieldNamesAudit = { "Input_UserName: ", "Response_GeoId: ",
										"Input_CountryShortName: ", "Input_CountryFullame: ",
										"Input_organizationStandardCode: ", "Input_effectiveDate: ",
										"Input_expirationDate: ", "Input_LastUpdateUserName: ",
										"Expected RevisionType CD:" };

								writableInputFields = Miscellaneous.geoFieldInputNames(inputAuditFieldValues,
										inputFieldNamesAudit);
								String[] dbFieldNamesAudit = { "DB_UserName: ", "DB_GeoId: ", "DB_CountryShortName: ",
										"DB_CountryFullame: ", "DB_organizationStandardCode: ", "DB_effectiveDate: ",
		...
		try {
			...
			if (Wscode == 400 && meta != null && (!meta.contains("timestamp"))
				...
				if (errorMsg1.get(0).equals("countryCode") && errorMsg2.get(0).equals(expectMessage)) {
					String[] inputFieldValues = { userId, countryCode, countryShortName, countryFullName,
							organizationStandardCode, effectiveDate, expirationDate };
					String[] inputFieldNames = { "Input_UserName: ", "Input_CountryCode: ", "Input_CountryShortName: ",
							"Input_CountryFullame: ", "Input_organizationStandardCode", "Input_effectiveDate",
					...
					logger.info("------------------------------------------------------------------");
					test.pass(
							"Expected error message is getting received in response when sending the blank Country Code");
					ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted,
	...
	public void TC_12() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.cntryOrgStdPostRequestWithoutMeta(userId, countryCode, countryShortName,
					countryFullName, organizationStandardCode, effectiveDate, expirationDate);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			logger.info("Input Request created");
			test.info(reqFormatted.replaceAll("\n", "<br />"));
	...
	public void TC_13() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.cntryOrgStdPostRequestWithoutCntryCd(userId, countryCode, countryShortName,
					countryFullName, organizationStandardCode, effectiveDate, expirationDate);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			logger.info("Input Request created");
	...
	public void TC_14() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.cntryOrgStdPostRequestWithoutCntryShNm(userId, countryCode, countryShortName,
					countryFullName, organizationStandardCode, effectiveDate, expirationDate);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			logger.info("Input Request created");
	...
	public void TC_15() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.cntryOrgStdPostRequestWithoutCntryFNm(userId, countryCode, countryShortName,
					countryFullName, organizationStandardCode, effectiveDate, expirationDate);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			logger.info("Input Request created");
	...
	public void TC_16() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.cntryOrgStdPostRequestWithoutOrgStdCd(userId, countryCode, countryShortName,
					countryFullName, organizationStandardCode, effectiveDate, expirationDate);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			logger.info("Input Request created");
	...
	public void TC_17() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.cntryOrgStdPostRequestWithoutEffectiveDate(userId, countryCode,
					countryShortName, countryFullName, organizationStandardCode, effectiveDate, expirationDate);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			logger.info("Input Request created");
			test.info(reqFormatted.replaceAll("\n", "<br />"));
	...
	public void TC_18() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.cntryOrgStdPostRequestWithoutExpirationDate(userId, countryCode,
					countryShortName, countryFullName, organizationStandardCode, effectiveDate, expirationDate);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			logger.info("Input Request created");
		...
		try {
			...
			if (Wscode == 400 && meta != null && (!meta.contains("timestamp"))
				...
				if (errorMsg1.get(0).equals("countryShortName") && errorMsg2.get(0).equals(expectMessage)) {
					...
					test.fail("Expected error message is not getting received as expected in response");
					ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "",
							Wsstatus, "" + Wscode, responsestr, "Fail", "countryShortName" + expectMessage);
					Assert.fail("Test Failed");
				}
			} else {
				if (Wscode != 400) {
				...
				ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "", Wsstatus,
						"" + Wscode, responsestr, "Fail", "countryShortName" + expectMessage);
				Assert.fail("Test Failed");
			}
		} catch (Exception e) {
			e.printStackTrace();
		...
		try {
			...
			if (Wscode == 400 && meta != null && (!meta.contains("timestamp"))
				...
				if (errorMsg1.get(0).equals("countryFullName") && errorMsg2.get(0).equals(expectMessage)) {
					...
					test.fail("Expected error message is not getting received as expected in response");
					ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "",
							Wsstatus, "" + Wscode, responsestr, "Fail", "countryFullName" + expectMessage);
					Assert.fail("Test Failed");
				}
			} else {
				if (Wscode != 400) {
				...
				ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "", Wsstatus,
						"" + Wscode, responsestr, "Fail", "countryFullName" + expectMessage);
				Assert.fail("Test Failed");
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void TC_27() {
		...
		try {
			...
			if (Wscode == 404) {
				...
				test.pass("Response status code 404 validation passed: " + Wscode);
				// ***error message validation
				String expectMessage = resMsgs.invalidUrlcountryOrgStdMsgstPost1;
				if (internalMsg.equals(expectMessage)) {
					String[] inputFieldValues = { userId, countryCode, countryShortName, countryFullName,
							organizationStandardCode, effectiveDate, expirationDate };
					String[] inputFieldNames = { "Input_UserName: ", "Input_CountryCode: ", "Input_CountryShortName: ",
	...
	public void TC_28() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.cntryOrgStdPostMissingCommaRequest(userId, countryCode, countryShortName,
					countryFullName, organizationStandardCode, effectiveDate, expirationDate);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			test.info(reqFormatted.replaceAll("\n", "<br />"));
	...
	public void TC_29() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			// String payload = PostMethod.holidaPostRequest(userId,
			// holidayName, holidayParamText);
	...
	public void TC_31() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			/*
			 * String str; // statement 1 str = new String("{" + "  \"data\": {"
			 ...
			 * String orgStdNm=getJMSResult.getString("orgStdNm"); String
			 * countryFullName=getJMSResult.getString("countryFullName"); String
			 * countryShortName=getJMSResult.getString("countryShortName");
			...
			// int
			// geopoliticalId1=getJMSResult.getJSONObject("data").getInt("geopoliticalId");
			if (getJMSResult != null) {
				...
				if (geopoliticalId != null) {
					// ***get the DB query
					String countryOrgStdJMSQuery = query.countryOrgStdJMStQuery(countryCode, countryShortName,
							countryFullName, orgStdNm, formatEffectiveDate, formatExpirationDate);
					// ***get the fields needs to be validate in DB
					List<String> fields = ValidationFields.countryOrgStdGetMethodDbFieldsJMS();
					// ***get the result from DB
					List<String> getResultDB = DbConnect.getResultSetFor(countryOrgStdJMSQuery, fields, fileName,
							testCaseID);
					String[] JMSValue = { geopoliticalId, orgStdNm, countryCode, countryFullName, countryShortName,
							effectiveDate, expirationDate };
					testResult = TestResultValidation.testValidationForJMS(JMSValue, getResultDB);

					if (testResult) {
						// ***write result to excel
						String[] responseDbFieldValues = { geopoliticalId, getResultDB.get(0), orgStdNm,
								getResultDB.get(1), countryCode, getResultDB.get(2), countryFullName,
								getResultDB.get(3), countryShortName, getResultDB.get(4), effectiveDate,
								getResultDB.get(5), expirationDate, getResultDB.get(6) };
						String[] responseDbFieldNames = { "Response_geopoliticalId: ", "DB_geopoliticalId: ",
								"Response_orgStdNm ", "DB_orgStdNm: ", "Response_countryCode: ", "DB_countryCode: ",
						...
						String[] responseDbFieldValues = { geopoliticalId, getResultDB.get(0), orgStdNm,
								getResultDB.get(1), countryCode, getResultDB.get(2), countryShortName,
								getResultDB.get(3), countryFullName, getResultDB.get(4), effectiveDate,
								getResultDB.get(5), expirationDate, getResultDB.get(6) };
						String[] responseDbFieldNames = { "Response_geopoliticalId: ", "DB_geopoliticalId: ",
								"Response_orgStdNm ", "DB_orgStdNm: ", "Response_countryCode: ", "DB_countryCode: ",
								"Response_countryFullName: ", "DB_countryFullName: ", "Response_countryShortName: ",
				...
				// logger.error("msgSource and DataSegement validation not
				// passed");
				logger.error("Execution is completed for Failed Test Case No. " + testCaseID);
				logger.error("------------------------------------------------------------------");
				test.fail("Posted request is not reached to JMS queue");
				ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, "", "", "", "", "", "", "Fail",
						"Posted request is not reached to JMS queue");
				Assert.fail("Test Failed");
			}
		} catch (Exception e) {
			e.printStackTrace();

...
public class DepnCountryRltspTypeGet extends Reporting {
	...
	protected void startRepo(Method m) throws IOException {
		...
		if (runFlag.equalsIgnoreCase("Yes")) {
			...
			String actuatorQueryVersionURL = RetrieveEndPoints.getEndPointUrl("queryActuator", fileName,
					level + ".query.version");
			// actuatorQueryVersion
			// =resultValidation.versionValidation(fileName, tokenKey,
			// tokenVal,actuatorQueryVersionURL);
		}
	}
	...
	public void TC_01() {
		...
		try {
			...
			// ***get end point url
			String getEndPoinUrl = RetrieveEndPoints.getEndPointUrl("geoGet", fileName,
					level + ".depnCntryRltspType.get");
			...
			if (Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS")
				...
				if (getResultDB.size() == responseRows.size() * fields.size()) {
					...
					for (int i = 0; i < responseRows.size(); i++) {
						if (StringUtils.isBlank(js.getString("data[" + i + "].dependentRelationshipTypeId"))) {
							getResponseRows.add("");
						} else {
							getResponseRows.add(js.getString("data[" + i + "].dependentRelationshipTypeId"));
						}
						if (StringUtils.isBlank(js.getString("data[" + i + "].dependentRelationshipTypeDescription"))) {
							getResponseRows.add("");
						} else {
							getResponseRows.add(js.getString("data[" + i + "].dependentRelationshipTypeDescription"));
						}
					}
					...
					for (int j = 0; j < getResultDB.size(); j = j + fields.size()) {
						if (getResultDB.get(j).toString().equals(getResponseRows.get(j).toString())
							...
							String[] responseDbFieldNames = { "Response_dependentRelationshipTypeId: ",
									"DB_dependentRelationshipTypeId: ",
									"Response_dependentRelationshipTypeDescription: ",
									"DB_dependentRelationshipTypeDescription: " };
...
//			test.info(responsestr1.replaceAll("\n", "<br />"));
	...
	public void TC_02() {
		...
		try {
			...
			if (Wscode == 404) {
				...
				String expectMessage = resMsgs.invalidUrlDepnCntryRltspTypeGet;
	...
	public void TC_04() {
		...
		try {
			...
			String meta = js.getString("meta");
			String actualRespVersionNum = js.getString("meta.version");
			if (Wscode == 401 && meta != null && actualRespVersionNum.equalsIgnoreCase(actuatorQueryVersion)) {

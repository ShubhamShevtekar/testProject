...
public class AffilTypePost extends Reporting {
			...
			if (Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS") && meta != null
				...
				if (js.getString("data.affiliationTypeId") != null) {
					...
					if (internalMsg.equals(expectMessage)) {
						logger.info("Affiliation Type ID is getting generated and received in response: "
								+ affiliationTypeId1);
						test.pass("Affiliation Type ID is getting generated and received in response: "
								+ affiliationTypeId1);
						// ***send the input, response, DB result for validation
						String[] inputFieldValues = { userId, affiliationTypeId1, affiliationTypeCode,
								affiliationTypeName, userId };
		...
		try {
			...
			if (Wscode == 400 && meta != null && actualRespVersionNum.equalsIgnoreCase(actuatorcommandversion)) {
				...
				if (errorMsg1.get(0).equals("affiliationTypeCode") && errorMsg2.get(0).equals(expectMessage)) {
					...
					String[] inputFieldNames = { "Input_UserName: ", "Input_affiliationTypeCode: ",
							"Input_affiliationTypeName: " };
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					logger.info(
	...
	public void TC_04() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.affilTypePostRequestWithoutMeta(userId, affiliationTypeCode,
					affiliationTypeName);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			test.info(reqFormatted.replaceAll("\n", "<br />"));
	...
	public void TC_05() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.affilTypePostRequestWithoutAffilTypeCd(userId, affiliationTypeCode,
					affiliationTypeName);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			test.info(reqFormatted.replaceAll("\n", "<br />"));
	...
	public void TC_11() {
		...
		try {
			...
			if (Wscode == 404) {
				...
				ValidationFields.transactionIdValidation(js, res);
				// ***error message validation
				String expectMessage = resMsgs.invalidUrlMsgAffilPOST;
	...
	public void TC_12() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.affilTypePostRequestMissingcomma(userId, affiliationTypeCode,
					affiliationTypeName);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			test.info(reqFormatted.replaceAll("\n", "<br />"));
				...
				if (affiliationTypeCode != null) {
					...
					if (testResult) {
						...
						String[] responseDbFieldNames = { "Response_affiliationTypeId: ", "DB_affiliationTypeId: ",
								"Response_affiliationTypeCode: ", "DB_affiliationTypeCode: ",

					...
					for (int j = 0; j < getResultDB.size(); j = j + fields.size()) {
						if (getResultDB.get(j).toString().equals(getResponseRows.get(j).toString())
							...
							String[] responseDbFieldNames = { "Response_holidayId: ", "DB_holidayId: ",
									"Response_holidayName: ", "DB_holidayName: ", "Response_holidayDateParameterText: ",
			...
			if (responseRows.size() == 0) {
				...
				if (Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS")
					...
					if (internalMsg.equals(expectMessage)) {
						...
						logger.info(
								"Expected error message is getting received in response when passing the invalid holidayName in URI");
	...
	public void TC_04() {
		...
		try {
			...
			if (Wscode == 404) {
				...
				// ***error message validation
				String expectMessage = resMsgs.invalidUrlHolidayGet;

...
public class DepnCountryRltspPut extends Reporting {

	String scenarioName = getClass().getSimpleName();
	String TestCaseDescription, scenarioType, userId, depnCntryRltspDesc, depnCntryRltspId;// ,
																							// token;
	...
	static Logger logger = Logger.getLogger(DepnCountryRltspPut.class);
	...
	public void TC_01() {
		...
		try {
			...
			// ***get end point url
			String getEndPoinUrl = RetrieveEndPoints.getEndPointUrl("geoPut", fileName, level + ".depnCntryRltsp.put");
			getEndPoinUrl = getEndPoinUrl + depnCntryRltspId;
			...
			if (Wscode == 200 && Wsstatus.equalsIgnoreCase("Success") && meta != null && (!meta.contains("timestamp"))
				...
				// ***get the DB query
				String depnCntryRltspPuttQuery = query.depnCntryRltspPostQuery(depnCntryRltspDesc);
				...
				List<String> getResultDB = DbConnect.getResultSetFor(depnCntryRltspPuttQuery, fields, fileName,
						testCaseID);
				if (js.getString("data.dependentRelationshipId") != null) {
					String depnCntryRltspId1 = js.getString("data.dependentRelationshipId");
					// ***Success message validation
					String expectMessage = resMsgs.depnCntryRltspPutSuccessMsg + depnCntryRltspId1;
					if (internalMsg.equals(expectMessage) && depnCntryRltspId.equals(depnCntryRltspId1)) {
						logger.info(
								"Success message with Dependent Country Relationship ID is getting received as expected in response");
						test.pass(
								"Success message with Dependent Country Relationship ID is getting received as expected in response");
					} else {
					...
					String[] inputFieldValues = { depnCntryRltspId, depnCntryRltspDesc, userId };
					...
					String[] inputFieldNames = { "Input_DepnCntryRltspId: ", "Input_DepnCntryRltspDesc: ",
							"Input_LastUpdateUserName: " };
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					String[] dbFieldNames = { "DB_DepnCntryRltspId: ", "DB_DepnCntryRltspDesc: ",
	...
	public void TC_02() {
		...
		try {
			...
			if (Wscode == 400 && meta != null && (!meta.contains("timestamp"))
				...
				if (errorMsg1.get(0).equals("dependentRelationshipDescription")
						&& errorMsg2.get(0).equals(expectMessage)) {
					logger.info(
							"Expected error message is getting received in response when sending the blank DepnCntryRltspDesc");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");

					String[] inputFieldValues = { userId, depnCntryRltspDesc };
					String[] inputFieldNames = { "Input_UserName: ", "Input_DepnCntryRltspDesc: " };
					writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
					test.pass(
							"Expected error message is getting received in response when sending the blank DepnCntryRltspDesc");
					ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted,
					...
					ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "",
							...
							"dependentRelationshipDescription" + expectMessage);
					Assert.fail("Test Failed");
				}
			} else {
				...
				ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "", Wsstatus,
						"" + Wscode, responsestr, "Fail", "dependentRelationshipDescription" + expectMessage);
				Assert.fail("Test Failed");
			}
		} catch (Exception e) {
			e.printStackTrace();
		...
		try {
			...
			if (Wscode == 400 && meta != null && (!meta.contains("timestamp"))
				...
				if (errorMsg1.get(0).equals("dependentRelationshipDescription")
					...
					logger.info(
							"Expected error message is getting received in response when the dependentRelationshipDescription attribute is not passed in JSON");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
		...
		try {
			...
			if (Wscode == 400 && meta != null && (!meta.contains("timestamp"))
				...
				if (errorMsg1.get(0).equals("dependentRelationshipDescription")
					...
					logger.info(
							"Expected error message is getting received in response when the dependentRelationshipDescription is more than 65 characters length in JSON");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
	...
	public void TC_08() {
		...
		try {
			...
			if (Wscode == 404) {
				...
				test.pass("Response status code 400 validation passed: " + Wscode);
				// ***error message validation
				String expectMessage = resMsgs.invalidUrldentcountryMsgstPut1;
		...
		try {
			...
			if (Wscode == 404 && meta != null && (!meta.contains("timestamp"))
				...
				if (errorMsg1.get(0).equals("Error") && errorMsg2.get(0).equals(expectMessage)) {
					logger.info(
							"Expected error message is getting received in response when the dependentRelationshipId is not exist in DB");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
	...
	public void TC_10() {
		...
		try {
			...
			if (Wscode == 404 && meta != null && (!meta.contains("timestamp"))
				...
				if (errorMsg1.get(0).equals("Error") && errorMsg2.get(0).equals(expectMessage)) {
					logger.info(
							"Expected error message is getting received in response when the alphabets/ special characters are provided dependentRelationshipId");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
	...
	public void TC_11() {
		...
		try {
			...
			if (Wscode == 404 && meta != null && (!meta.contains("timestamp"))
				...
				if (errorMsg1.get(0).equals("Error") && errorMsg2.get(0).equals(expectMessage)) {
					logger.info(
							"Expected error message is getting received in response when the special characters are provided dependentRelationshipId");
					logger.info("Execution is completed for Passed Test Case No. " + testCaseID);
					logger.info("------------------------------------------------------------------");
	...
	public void TC_12() {
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			// ***send the data to create request and get request
			String payload = PostMethod.depnCntryRltspMissingCommaPostRequest(userId, depnCntryRltspDesc);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			test.info(reqFormatted.replaceAll("\n", "<br />"));
			// ***get end point url
	...
	public void TC_15() {
		...
		try {
			...
			if (getJMSResult != null) {
				...
				String dependentRelationshipDescription = getJMSResult.getJSONObject("data")
						.getString("dependentRelationshipDescription");
				...
				if (dependentRelationshipDescription != null) {
					...
					// ***get the result from DB
					List<String> getResultDB = DbConnect.getResultSetFor(depnCntryRltspPostQuery, fields, fileName,
							testCaseID);
					...
					if (testResult) {
						// ***write result to excel
						String[] responseDbFieldValues = { dependentRelationshipIdJms, getResultDB.get(0),
								dependentRelationshipDescription, getResultDB.get(1) };
	...
	public void testDataFields(String scenarioName, String testCaseId) {
		...
		depnCntryRltspId = inputData1.get(testCaseId).get("DependentRelationshipId");
	}
}

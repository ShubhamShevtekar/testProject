		...
		try {
	        ...
	        if(Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS") && meta !=null)
	        {
	    		...
	    		if(getResultDB.size() == responseRows.size()*fields.size())
	    		{
		        	...
		        	{
		        		if(StringUtils.isBlank(js.getString("data["+i+"].orgStdCd")))
		        		{
		        			getResponseRows.add("");
		        		}else {
		        			getResponseRows.add(js.getString("data["+i+"].orgStdCd"));
		        		}
	        ...
	        if(responseRows.size()==0)
	        {
	        	 ...
	        	 if(Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS") && meta !=null)
		        {
					...
					if(internalMsg.equals(expectMessage))
					{
		        		...
		        		logger.info("Expected error message is getting received in response when passing the invalid orgStdCd in URI");
		        		logger.info("Execution is completed for Passed Test Case No. "+testCaseID);
		    			logger.info("------------------------------------------------------------------");
		        		test.pass("Expected error message is getting received in response when passing the invalid orgStdCd in URI");

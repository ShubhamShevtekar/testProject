...
public class HolidayGraphQL extends Reporting{
	
	String scenarioName = getClass().getSimpleName();
	String TestCaseDescription, scenarioType, holidayName;
	...
	static Logger logger = Logger.getLogger(HolidayGraphQL.class);
	...
	{	
		...
		try {
			...
			String payload = PostMethod.holidayGraphQl();
		    ...
		    List<String> responseRows = js.get("data.holidays");
	        ...
	        if(Wscode == 200 && meta!=null && (!meta.contains("timestamp"))&& actualRespVersionNum.equalsIgnoreCase("1.0.0"))
	        {
	    		...
	    		List<String> getResultDB = DbConnect.getResultSetFor(holidayGetQuery, fields, fileName, testCaseID);
	    		...
	    		if(getResultDB.size() == responseRows.size()*fields.size())
	    		{
		        	...
		        	{
		        		if(StringUtils.isBlank(js.getString("data.holidays.holidayId["+i+"]")))
		        		{
		        			getResponseRows.add("");
		        		}else {
		        			getResponseRows.add(js.getString("data.holidays.holidayId["+i+"]"));
		        		}
		        		if(StringUtils.isBlank(js.getString("data.holidays.holidayName["+i+"]")))
		        		{
		        			getResponseRows.add("");
		        		}else {
		        			getResponseRows.add(js.getString("data.holidays.holidayName["+i+"]"));
		        		}
		        		if(StringUtils.isBlank(js.getString("data.holidays.holidayDateParameterText["+i+"]")))
		        		{
		        			getResponseRows.add("");
		        		}else {
		        			getResponseRows.add(js.getString("data.holidays.holidayDateParameterText["+i+"]"));
		        		}
		        	}
		        	...
		        	for(int j=0; j<getResultDB.size(); j=j+fields.size())
		        	{
		        		if(getResultDB.get(j).toString().equals(getResponseRows.get(j).toString()) && getResultDB.get(j+1).toString().equals(getResponseRows.get(j+1).toString()))
		        		{
		        			...
		        			String[] responseDbFieldNames = {"Response_holidayId: ", "DB_holidayId: ", 
		        					"Response_holidayName: ", "DB_holidayName: ", "Response_holidayDateParameterText: ", "DB_holidayDateParameterText: "};
	...
	{	
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			//***send the data to create request and get request
			String payload = PostMethod.holidayParameterGraphQl(holidayName);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			logger.info("Input Request created");
			test.info(reqFormatted.replaceAll("\n", "<br />"));
	...
	{	
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			//***send the data to create request and get request
			String payload = PostMethod.holidayParameterWithAttributeGraphQl(holidayName);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			logger.info("Input Request created");
			test.info(reqFormatted.replaceAll("\n", "<br />"));
	       ...
	       // String internalMsg = js.getString("meta.message.internalMessage");
	        ...
	        if(Wscode == 200 && meta!=null && (!meta.contains("timestamp"))&& actualRespVersionNum.equalsIgnoreCase("1.0.0"))
	        {
	        	...
	        	test.pass("Response timestamp validation passed");test.pass("Response API version number validation passed");
	        	//***get the DB query
	        	String holidayPostQuery = query.holidayGraphQlQuery(holidayName);
	    		//***get the fields needs to be validate in DB
	    		List<String> fields = ValidationFields.holidayGetMethodDbGrpgQLFields();
	    		//***get the result from DB
	    		List<String> getResultDB = DbConnect.getResultSetFor(holidayPostQuery, fields, fileName, testCaseID);
		...
		try {
	       ...
	       // String internalMsg = js.getString("meta.message.internalMessage");
	        ...
	        String internalMsg = js.getString("meta.message.internalMessage");
    			...
    			{
    				logger.info("Holiday records are not available for this holiday name: "+holidayName);
		        	test.info("Holiday records are not available for this holiday name: "+holidayName);
		        	test.pass("Holiday records are not available for this holiday name: "+holidayName);
		        	ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "", "", ""+Wscode,
							responsestr1, "Pass", internalMsg );
    			}
	    		else
	    		{
	    			logger.error("Holiday records are available for this holiday name: "+holidayName);
					logger.error("Execution is completed for Failed Test Case No. "+testCaseID);
					logger.error("------------------------------------------------------------------");
		        	test.fail("Holiday records are available for this holiday name: "+holidayName);
		        	ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted, "", "", "", ""+Wscode,
							responsestr1, "Fail", internalMsg );
		        	Assert.fail("Test Failed");
	    		}
	...
	{	
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			//***send the data to create request and get request
			String payload = PostMethod.holidayParameterOneWrongAttrbuteGraphQl(holidayName);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			logger.info("Input Request created");
			test.info(reqFormatted.replaceAll("\n", "<br />"));
	        ...
	        String expectMessage = resMsgs.ErrorMsg;
			...
			if(Wscode == 200 &&  meta != null && (!meta.contains("timestamp"))&& actualRespVersionNum.equalsIgnoreCase("1.0.0"))
	    		...
	    		if(errorMsg1.get(0).equals("ValidationError") && errorMsg2.get(0).equals(expectMessage))
	    		{
	        		...
	        		logger.info("Expected error message is getting received in response when sending passing the invalid any one of the attribute name");
	...
	{	
		...
		try {
			...
			test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
			//***send the data to create request and get request
			String payload = PostMethod.holidayParameterTwoeWrongAttrbuteGraphQl(holidayName);
			String reqFormatted = Miscellaneous.jsonFormat(payload);
			test.info("Input Request created:");
			logger.info("Input Request created");
			test.info(reqFormatted.replaceAll("\n", "<br />"));
	       ...
	       // String internalMsg = js.getString("meta.message.internalMessage");
	        ...
	        String internalMsg = js.getString("meta.message.internalMessage");
	        int errrorMsgLength=js.get("meta.errors.size");
	        String expectMessag1 = resMsgs.HolidayErrorMsg1;
	        String expectMessage2 = resMsgs.HolidayErrorMsg2;
	        List<String> errorMsg1 = new ArrayList<String>();
			List<String> errorMsg2 = new ArrayList<String>();
			for(int i=0;i<errrorMsgLength;i++){
				errorMsg1.add(js.getString("meta.errors["+i+"].error"));
			...
			if(Wscode == 200 &&  meta != null && (!meta.contains("timestamp"))&& actualRespVersionNum.equalsIgnoreCase("1.0.0"))
	    		...
	    		if ((errorMsg1.get(0).equals("ValidationError") && errorMsg2.get(0).equals(expectMessag1))&&(errorMsg1.get(1).equals("ValidationError") && errorMsg2.get(1).equals(expectMessage2)))
	    		{
					...
					test.fail("Expected error message is not getting received in response");
		        	ex.writeExcel(fileName, testCaseID, TestCaseDescription, scenarioType, reqFormatted,"", "", "", ""+Wscode,
							responsestr, "Fail", "orgStdCd"+expectMessag1 );
		        	Assert.fail("Test Failed");

...
public class GeoRltspTypeGet extends Reporting{
	...
	String TestCaseDescription, scenarioType, relationshipTypeCode;
	        ...
	        if(Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS"))
	        {
	    		...
	    		if(getResultDB.size() == responseRows.size()*fields.size())
	    		{
		        	...
		        	for(int i=0; i<responseRows.size(); i++)
		        	{
		        		...
		        		if(StringUtils.isBlank(js.getString("data["+i+"].areaRelationshipTypeDesc")))
		        		{
		        			getResponseRows.add("");
		        		}else {
		        			getResponseRows.add(js.getString("data["+i+"].areaRelationshipTypeDesc"));
		        		}
		        	}
		        	...
		        	for(int j=0; j<getResultDB.size(); j=j+fields.size())
		        	{
		        		if(getResultDB.get(j).toString().equals(getResponseRows.get(j).toString()) && getResultDB.get(j+1).toString().equals(getResponseRows.get(j+1).toString()))
		        		{
		        			...
		        			String[] responseDbFieldNames = {"Response_relationshipTypeCode: ", "DB_relationshipTypeCode: ", 
		        					"Response_areaRelationshipTypeDesc: ", "DB_areaRelationshipTypeDesc: "};
	...
	{	
		...
		try {
			...
			//***get end point url
			String getEndPoinUrl = RetrieveEndPoints.getEndPointUrl("geoGet", fileName, level+".geoRsType.get");
			getEndPoinUrl = getEndPoinUrl+relationshipTypeCode;
			logger.info("URI passed: "+getEndPoinUrl);
        	test.pass("URI passed: "+getEndPoinUrl);
			//***send request and get response
			Response res = GetResponse.sendRequestGet(tokenValues[0], token, getEndPoinUrl, fileName, testCaseID);
		...
		try {
	        ...
	        if(responseRows.size()==0)
	        {
		        ...
		        if(Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS"))
		        {
					...
					{
						String[] inputFieldValues = {relationshipTypeCode};
						String[] inputFieldNames = {"Input_relationshipTypeCode: "};
		        		writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
		        		logger.info("Expected error message is getting received in response when passing the invalid relationshipTypeCode in URI");
		        		logger.info("Execution is completed for Passed Test Case No. "+testCaseID);
		    			logger.info("------------------------------------------------------------------");
		        		test.pass("Expected error message is getting received in response when passing the invalid relationshipTypeCode in URI");

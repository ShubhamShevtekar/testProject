...
public class HolidayGet extends Reporting{
	...
	{	
		...
		try {
			...
			//***get end point url
			String getEndPoinUrl = RetrieveEndPoints.getEndPointUrl("geoGet", fileName, level+".holiday.get");
	        ...
	        if(Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS") && actualRespVersionNum.equalsIgnoreCase("1.0.0"))
	        {
	    		...
	    		if(getResultDB.size() == responseRows.size()*fields.size())
	    		{
		        	...
		        	{
		        		if(StringUtils.isBlank(js.getString("data["+i+"].holidayId")))
		        		{
		        			getResponseRows.add("");
		        		}else {
		        			getResponseRows.add(js.getString("data["+i+"].holidayId"));
		        		}
		        		if(StringUtils.isBlank(js.getString("data["+i+"].holidayName")))
		        		{
		        			getResponseRows.add("");
		        		}else {
		        			getResponseRows.add(js.getString("data["+i+"].holidayName"));
		        		}
		        		if(StringUtils.isBlank(js.getString("data["+i+"].holidayDateParamText")))
		        		{
		        			getResponseRows.add("");
		        		}else {
		        			getResponseRows.add(js.getString("data["+i+"].holidayDateParamText"));
		        		}
		        	}
	...
	{	
		...
		try {
			...
			//***get end point url
			String getEndPoinUrl = RetrieveEndPoints.getEndPointUrl("geoGet", fileName, level+".holiday.get");
			getEndPoinUrl = getEndPoinUrl+holidayName;
			logger.info("URI passed: "+getEndPoinUrl);
        	test.pass("URI passed: "+getEndPoinUrl);
			//***send request and get response
			Response res = GetResponse.sendRequestGet(tokenValues[0], token, getEndPoinUrl, fileName, testCaseID);
		...
		try {
	        ...
	        if(responseRows.size()==0)
	        {
		        ...
		        if(Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS") && actualRespVersionNum.equalsIgnoreCase("1.0.0"))
		        {
					...
					{
						String[] inputFieldValues = {holidayName};
						String[] inputFieldNames = {"Input_holidayName: "};
		        		writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
		        		logger.info("Expected error message is getting received in response when passing the invalid holidayName in URI");
		        		logger.info("Execution is completed for Passed Test Case No. "+testCaseID);
		    			logger.info("------------------------------------------------------------------");
		        		test.pass("Expected error message is getting received in response when passing the invalid holidayName in URI");
	...
	{	
		...
		try {
	        ...
	        if(Wsstatus.equals("404") && timestamp!=null)
	        {
	        	...
	        	test.pass("Response timestamp validatrion passed");
	        	//***error message validation

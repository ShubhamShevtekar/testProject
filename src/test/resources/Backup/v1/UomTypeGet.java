...
public class UomTypeGet extends Reporting{
	...
	String TestCaseDescription, scenarioType, uomTypeCode;
		...
		try {
	        		...
	        		&& actualRespVersionNum.equalsIgnoreCase(actuatorQueryVersion))
	        {
	    		...
	    		if(getResultDB.size() == responseRows.size()*fields.size())
	    		{
		        	...
		        	{
		        		if(StringUtils.isBlank(js.getString("data["+i+"].uomTypeCode")))
		        		{
		        			getResponseRows.add("");
		        		}else {
		        			getResponseRows.add(js.getString("data["+i+"].uomTypeCode"));
		        		}
		        		if(StringUtils.isBlank(js.getString("data["+i+"].uomTypeName")))
		        		{
		        			getResponseRows.add("");
		        		}else {
		        			getResponseRows.add(js.getString("data["+i+"].uomTypeName"));
		        		}
		        		if(StringUtils.isBlank(js.getString("data["+i+"].uomTypeDescription")))
		        		{
		        			getResponseRows.add("");
		        		}else {
		        			getResponseRows.add(js.getString("data["+i+"].uomTypeDescription"));
		        		}
		        	}
	...
	{	
		...
		try {
			...
			//***get end point url
			String getEndPoinUrl = RetrieveEndPoints.getEndPointUrl("geoGet", fileName, level+".uomType.get");
			getEndPoinUrl = getEndPoinUrl+uomTypeCode;
			logger.info("URI passed: "+getEndPoinUrl);
        	test.pass("URI passed: "+getEndPoinUrl);
			//***send request and get response
			Response res = GetResponse.sendRequestGet(tokenValues[0], token, getEndPoinUrl, fileName, testCaseID);
	        ...
	        String meta =  js.getString("meta");
	        String actualRespVersionNum = js.getString("meta.version"); 
	        if(Wscode == 200 && Wsstatus.equalsIgnoreCase("SUCCESS") && meta !=null&& actualRespVersionNum.equalsIgnoreCase(actuatorQueryVersion))
	        {
	        	logger.info("Response status validation passed: "+Wscode);
	        	test.pass("Response status validation passed: "+Wscode);
	        	test.pass("Response meta validation passed");
		...
		try {
	        ...
	        if(responseRows.size()==0)
	        {
		        		...
		        		&& actualRespVersionNum.equalsIgnoreCase(actuatorQueryVersion))
		        {
					...
					{
						String[] inputFieldValues = {uomTypeCode};
						String[] inputFieldNames = {"Input_uomTypeCode: "};
		        		writableInputFields = Miscellaneous.geoFieldInputNames(inputFieldValues, inputFieldNames);
		        		logger.info("Expected error message is getting received in response when passing the invalid uomTypeCode in URI");
		        		logger.info("Execution is completed for Passed Test Case No. "+testCaseID);
		    			logger.info("------------------------------------------------------------------");
		        		test.pass("Expected error message is getting received in response when passing the invalid uomTypeCode in URI");

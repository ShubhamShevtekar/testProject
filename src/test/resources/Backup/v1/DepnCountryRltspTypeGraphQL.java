...
public class DepnCountryRltspTypeGraphQL extends Reporting{
	...
	static Logger logger = Logger.getLogger(DepnCountryRltspTypeGraphQL.class);
	...
	public void before(){
		...
		// *** getting actautor version DepnCountryRltspTypeGraphQL 
	...
	public void TC_01() {
			...
			try {
				...
				String payload = PostMethod.depnCntryRltspTypeGraphQL();
		       ...
		       // String internalMsg = js.getString("meta.message.internalMessage");
			    List<String> responseRows = js.get("data.dependentCountryRelationshipTypes");
	        ...
	        if(Wscode == 200 && meta!=null && (!meta.contains("timestamp"))&& actualRespVersionNum.equalsIgnoreCase(actuatorGraphQLversion))
	        {
	    		...
	    		if(getResultDB.size() == responseRows.size()*fields.size())
	    		{
		        	...
		        	{
		        		if(StringUtils.isBlank(js.getString("data.dependentCountryRelationshipTypes.dependentRelationshipId["+i+"]")))
		        		{
		        			getResponseRows.add("");
		        		}else {
		        			getResponseRows.add(js.getString("data.dependentCountryRelationshipTypes.dependentRelationshipId["+i+"]"));
		        		}
		        		if(StringUtils.isBlank(js.getString("data.dependentCountryRelationshipTypes.dependentRelationshipDescription["+i+"]")))
		        		{
		        			getResponseRows.add("");
		        		}else {
		        			getResponseRows.add(js.getString("data.dependentCountryRelationshipTypes.dependentRelationshipDescription["+i+"]"));
		        		}
		        	}
	...
	public void TC_02() {
			...
			try {
				...
				test.log(Status.INFO, MarkupHelper.createLabel(TestCaseDescription, ExtentColor.PURPLE));
				//***send the data to create request and get request
				String payload = PostMethod.depnCntryRltspTypeGraphQLWithWrongAttributes();
				String reqFormatted = Miscellaneous.jsonFormat(payload);
				test.info("Input Request created:");
				logger.info("Input Request created");
				test.info(reqFormatted.replaceAll("\n", "<br />"));
